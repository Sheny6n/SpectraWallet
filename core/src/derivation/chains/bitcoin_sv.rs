//! Bitcoin SV: address validation, BIP-32 derivation, legacy P2PKH
...
/// Derive a Bitcoin SV legacy P2PKH address. Mainnet uses 0x00; testnet 0x6f.
pub(crate) fn derive(request: ParsedRequest) -> Result<DerivedOutput, String> {
    ...
    let version: u8 = if matches!(request.chain, Chain::BitcoinSvTestnet) { 0x6f } else { 0x00 };
