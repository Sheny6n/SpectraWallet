...
/// Derive a Dogecoin keypair + address. Mainnet uses version byte 0x1e
/// (D… addresses); testnet uses 0x71.
pub(crate) fn derive(request: ParsedRequest) -> Result<DerivedOutput, String> {
    ...
    let version: u8 = if matches!(request.chain, Chain::DogecoinTestnet) { 0x71 } else { 0x1e };
