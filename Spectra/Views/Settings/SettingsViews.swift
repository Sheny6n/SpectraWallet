    ...
    var body: some View {
        Form {
            ...
            Section(localizedSettingsString("Fiat Rate Provider")) {
                ...
                .pickerStyle(.menu)
                Text(copy.fiatRateProviderNote)
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
...
struct PriceAlertsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section(localizedSettingsString("New Alert")) {
                if store.alertableCoins.isEmpty {
                    Text(localizedSettingsString("Import a wallet with assets first. Alerts are created from assets currently in your portfolio."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
                    ...
                    if let formMessage {
                        Text(formMessage)
                            .font(.caption)
                            .foregroundStyle(isDuplicateDraftAlert ? .orange : .secondary)
                    }
            ...
            Section(localizedSettingsString("Active Alerts")) {
                if store.priceAlerts.isEmpty {
                    Text(localizedSettingsString("No alerts configured yet."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
                            ...
                            HStack {
                                VStack(alignment: .leading, spacing: 2) {
                                    Text(alert.titleText)
                                        .font(.headline)
                                    Text("\(alert.condition.displayName) \(store.formattedFiatAmount(fromUSD: alert.targetPrice))")
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                        .spectraNumericTextLayout()
                                }
                                ...
                                Text(alert.statusText)
                                    ...
                                    .padding(.vertical, 4)
                                    .background(statusColor(for: alert).opacity(0.18), in: Capsule())
        ...
        .navigationTitle(localizedSettingsString("Price Alerts"))
        .onAppear {
            syncSelection()
        }
    ...
    private var canAddAlert: Bool {
        guard selectedCoin != nil,
              let targetPrice = Double(targetPriceText.trimmingCharacters(in: .whitespacesAndNewlines)),
              targetPrice > 0 else {
            return false
        }
    ...
    private var normalizedDraftTargetPrice: Double? {
        guard let targetPriceInSelectedFiat = Double(targetPriceText.trimmingCharacters(in: .whitespacesAndNewlines)),
              targetPriceInSelectedFiat > 0 else {
            return nil
        }
    ...
    private var isDuplicateDraftAlert: Bool {
        guard let selectedCoin,
              let normalizedDraftTargetPrice else {
            return false
        }
...
struct AddressBookView: View {
    ...
    private var addressPrompt: String {
        ...
        case "Internet Computer":
            return "64-char account identifier"
        case "NEAR":
            return "alice.near / 64-char hex"
        case "Polkadot":
            return "1..."
        case "Stellar":
            return "G..."
    ...
    var body: some View {
        Form {
            ...
            Section(localizedSettingsString("New Contact")) {
                TextField(localizedSettingsString("Name"), text: $contactName)
                    .textInputAutocapitalization(.words)
                    .autocorrectionDisabled()

                Picker(localizedSettingsString("Chain"), selection: $selectedChainName) {
                    ForEach(supportedChains, id: \.self) { chainName in
                        Text(chainName).tag(chainName)
                    }
                }

                TextField(addressPrompt, text: $address)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                ...
                TextField(localizedSettingsString("Note (Optional)"), text: $note)
                ...
                if let formMessage {
                    Text(formMessage)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .foregroundColor(store.canSaveAddressBookEntry(name: contactName, address: address, chainName: selectedChainName) ? nil : .red)
                }
            ...
            Section(localizedSettingsString("Saved Addresses")) {
                if store.addressBook.isEmpty {
                    Text(localizedSettingsString("No saved recipients yet."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            HStack(alignment: .top, spacing: 12) {
                                ...
                                Button {
                                    ...
                                    Label(copiedEntryID == entry.id ? localizedSettingsString("Copied") : localizedSettingsString("Copy"), systemImage: copiedEntryID == entry.id ? "checkmark" : "doc.on.doc")
        ...
        .navigationTitle(localizedSettingsString("Address Book"))
        .sheet(item: $editingEntry) { entry in
            NavigationView {
                Form {
                    Section {
                        Text(localizedSettingsString("You can update the label for this saved address. The chain, address, and note stay fixed."))
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    ...
                    Section(localizedSettingsString("Label")) {
                        TextField(localizedSettingsString("Name"), text: $editedName)
                            .textInputAutocapitalization(.words)
                            .autocorrectionDisabled()
                    }
                }
                .navigationTitle(localizedSettingsString("Edit Label"))
                .toolbar {
...
struct AboutView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 22) {
                    ...
                    aboutCard(
                        title: copy.aboutEthosTitle,
                        lines: copy.aboutEthosLines
                    )
        ...
        .navigationTitle(localizedSettingsString("About Spectra"))
        ...
        VStack(spacing: 18) {
            ZStack {
                Circle()
                    ...
                    .blur(radius: 26)
                    .rotationEffect(.degrees(isAnimatingHero ? 360 : 0))
                    .animation(.linear(duration: 18).repeatForever(autoreverses: false), value: isAnimatingHero)
...
struct BackgroundSyncSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section(localizedSettingsString("Hint")) {
                Label(localizedSettingsString("Lower refresh times can increase battery usage and network traffic."), systemImage: "bolt.batteryblock.fill")
            ...
            if isTooFrequent(store.automaticRefreshFrequencyMinutes) {
                Section(localizedSettingsString("Warning")) {
                    Label(localizedSettingsString("This refresh speed can increase battery usage and network traffic."), systemImage: "exclamationmark.triangle.fill")
                        .foregroundStyle(.orange)
                    Text(localizedSettingsString("Use this mode only if you need near-real-time updates."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private func isTooFrequent(_ minutes: Int) -> Bool {
        minutes <= 10
    }
}
...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(localizedSettingsString("Settings"))
            .navigationDestination(for: Route.self) { route in
                switch route {
                case .addressBook:
                    AddressBookView(store: store)
                case .trackedTokens:
                    TokenRegistrySettingsView(store: store)
                case .feePriorities:
                    ChainFeePrioritySettingsView(store: store)
                case .iconStyles:
                    TokenIconSettingsView()
                case .decimalDisplay:
                    DecimalDisplaySettingsView(store: store)
                case .refreshFrequency:
                    BackgroundSyncSettingsView(store: store)
                case .priceAlerts:
                    PriceAlertsView(store: store)
                case .largeMovementAlerts:
                    LargeMovementAlertsSettingsView(store: store)
                case .pricing:
                    PricingSettingsView(store: store)
                case .endpoints:
                    EndpointCatalogSettingsView(store: store)
                case .diagnostics:
                    DiagnosticsHubView(store: store)
                case .operationalLogs:
...
struct ReportProblemView: View {
    ...
    private var reportProblemURL: URL {
        URL(string: copy.reportProblemURL) ?? URL(string: "https://example.com/spectra/report-problem")!
    }

    var body: some View {
        Form {
            Section {
                Text(copy.reportProblemDescription)
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Section(localizedSettingsString("Support Link")) {
                ...
                Text(reportProblemURL.absoluteString)
                    .font(.caption.monospaced())
                    .foregroundStyle(.secondary)
                    .textSelection(.enabled)
            }
...
struct BuyCryptoHelpView: View {
    ...
    var body: some View {
        Form {
                ...
                ForEach(providers) { provider in
                    VStack(alignment: .leading, spacing: 8) {
                        Link(destination: provider.url) {
                            Label(provider.name, systemImage: "arrow.up.right.square")
                                .font(.headline)
                        }
            ...
            Section(localizedSettingsString("Reminder")) {
                Text(copy.buyWarning)
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
...
struct AdvancedSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section(localizedSettingsString("Quick Maintenance")) {
                ...
                Button(refreshButtonTitle(for: "Bitcoin")) {
                    refreshSingleChain("Bitcoin")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Litecoin")) {
                    refreshSingleChain("Litecoin")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Dogecoin")) {
                    refreshSingleChain("Dogecoin")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Ethereum")) {
                    refreshSingleChain("Ethereum")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Ethereum Classic")) {
                    refreshSingleChain("Ethereum Classic")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Arbitrum")) {
                    refreshSingleChain("Arbitrum")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Optimism")) {
                    refreshSingleChain("Optimism")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "BNB Chain")) {
                    refreshSingleChain("BNB Chain")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Avalanche")) {
                    refreshSingleChain("Avalanche")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Hyperliquid")) {
                    refreshSingleChain("Hyperliquid")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Tron")) {
                    refreshSingleChain("Tron")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Solana")) {
                    refreshSingleChain("Solana")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Cardano")) {
                    refreshSingleChain("Cardano")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "XRP Ledger", label: "XRP")) {
                    refreshSingleChain("XRP Ledger")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Monero")) {
                    refreshSingleChain("Monero")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Sui")) {
                    refreshSingleChain("Sui")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Aptos")) {
                    refreshSingleChain("Aptos")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "TON")) {
                    refreshSingleChain("TON")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Internet Computer")) {
                    refreshSingleChain("Internet Computer")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "NEAR")) {
                    refreshSingleChain("NEAR")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Polkadot")) {
                    refreshSingleChain("Polkadot")
                }
                .disabled(isRunningMaintenance)
                Button(refreshButtonTitle(for: "Stellar")) {
                    refreshSingleChain("Stellar")
                }
                .disabled(isRunningMaintenance)
            ...
            Section(localizedSettingsString("Status")) {
                Text(store.networkSyncStatusText)
                    .font(.caption)
                    .foregroundStyle(.secondary)
                ...
                Text(localizedSettingsFormat("Wallets: %lld", store.wallets.count))
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        .navigationTitle(localizedSettingsString("Advanced"))
        .sheet(isPresented: $isShowingDiagnosticsExportsBrowser) {
            DiagnosticsExportsBrowserView(store: store)
        }
        .fileImporter(
            isPresented: $isShowingDiagnosticsImporter,
    ...
    private func refreshButtonTitle(for chainName: String, label: String? = nil) -> String {
        ...
        return isRunningMaintenance
            ? localizedSettingsFormat("Refreshing %@...", title)
            : localizedSettingsFormat("Refresh %@", title)
    }
}
...
struct DiagnosticsExportsBrowserView: View {
        ...
        NavigationStack {
            List {
                if exportURLs.isEmpty {
                    Text(localizedSettingsString("No diagnostics exports yet."))
                        .foregroundStyle(.secondary)
                } else {
            ...
            .navigationTitle(localizedSettingsString("Past Exports"))
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
    ...
    private func reloadExports() {
        exportURLs = store.diagnosticsBundleExportURLs()
    }
...
struct LargeMovementAlertsSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                Text(localizedSettingsString("These controls tune when portfolio movement notifications are sent during portfolio balance refreshes."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
...
struct TokenRegistrySettingsView: View {
    ...
    private enum TokenRegistryChainFilter: CaseIterable, Identifiable {
        ...
        var title: String {
            switch self {
            case .all:
                return localizedSettingsString("All")
            case .ethereum:
                return TokenTrackingChain.ethereum.filterDisplayName
            case .arbitrum:
                return TokenTrackingChain.arbitrum.filterDisplayName
            case .optimism:
                return TokenTrackingChain.optimism.filterDisplayName
            case .bnb:
                return TokenTrackingChain.bnb.filterDisplayName
            case .avalanche:
                return TokenTrackingChain.avalanche.filterDisplayName
            case .hyperliquid:
                return TokenTrackingChain.hyperliquid.filterDisplayName
            case .solana:
                return TokenTrackingChain.solana.filterDisplayName
            case .sui:
                return TokenTrackingChain.sui.filterDisplayName
            case .aptos:
                return TokenTrackingChain.aptos.filterDisplayName
            case .ton:
                return TokenTrackingChain.ton.filterDisplayName
            case .near:
                return TokenTrackingChain.near.filterDisplayName
            case .tron:
                return TokenTrackingChain.tron.filterDisplayName
            }
        }
    ...
    private enum TokenRegistrySourceFilter: CaseIterable, Identifiable {
        ...
        var title: String {
            ...
            case .all:
                return localizedSettingsString("All")
            case .builtIn:
                return localizedSettingsString("Built-In")
            case .custom:
                return localizedSettingsString("Custom")
            }
        }
    }
    ...
    var body: some View {
        Form {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    VStack(spacing: 10) {
                        Picker(localizedSettingsString("Network"), selection: $chainFilter) {
                            ForEach(TokenRegistryChainFilter.allCases) { filter in
                        ...
                        Picker(localizedSettingsString("Source"), selection: $sourceFilter) {
                            ForEach(TokenRegistrySourceFilter.allCases) { filter in
                                Text(filter.title).tag(filter)
                            }
                        }
                        .pickerStyle(.menu)
            ...
            Section(localizedSettingsString("Tracked Tokens")) {
                if filteredGroups.isEmpty {
                    Text(searchText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? localizedSettingsString("No tracked tokens match the selected filters.") : localizedSettingsString("No matching tokens."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
        ...
        .navigationTitle(localizedSettingsString("Tracked Tokens"))
        .toolbar {
    ...
    private var filteredGroups: [TokenRegistryGroup] {
        ...
        let groups = grouped.values.compactMap { entries -> TokenRegistryGroup? in
            ...
            return TokenRegistryGroup(
                key: TokenRegistryGrouping.key(for: representative),
        ...
        let filtered = groups.filter { group in
            if let selectedChain = chainFilter.chain,
               !group.entries.contains(where: { $0.chain == selectedChain }) {
                return false
            }
            ...
            case .all:
                break
            case .builtIn:
                guard group.entries.contains(where: \.isBuiltIn) else { return false }
            case .custom:
                guard group.entries.contains(where: { !$0.isBuiltIn }) else { return false }
            }
            ...
            let haystack = (
                ...
                group.entries.flatMap { entry in
                    [entry.chain.rawValue, entry.tokenStandard, entry.contractAddress, entry.coinGeckoID]
                }
            )
            .joined(separator: " ")
            .lowercased()
...
struct TokenRegistryDetailView: View {
    ...
    private var groupEntries: [TokenPreferenceEntry] {
        store.resolvedTokenPreferences
            .filter { TokenRegistryGrouping.key(for: $0) == groupKey }
            .sorted { lhs, rhs in
                if lhs.chain != rhs.chain {
                    return lhs.chain.rawValue < rhs.chain.rawValue
                }
    ...
    private var representativeEntry: TokenPreferenceEntry? {
        groupEntries.first
    }
                    ...
                    Section {
                        HStack(spacing: 12) {
                            CoinBadge(
                                ...
                                color: settingsTokenTint(for: representativeEntry.chain),
                                size: 42
                            )

                            VStack(alignment: .leading, spacing: 4) {
                                Text(representativeEntry.name)
                                    .font(.headline)
                                Text(representativeEntry.symbol)
                                    .font(.subheadline)
                                    .foregroundStyle(.secondary)
                            }
                        }
...
struct AddCustomTokenView: View {
    ...
    var body: some View {
        Form {
            Section {
                Text(localizedSettingsString("Add a custom token contract, mint address, coin type, package address, account ID, or jetton master address for Ethereum, Arbitrum, Optimism, BNB Chain, Avalanche, Hyperliquid, Solana, Sui, Aptos, TON, NEAR, or Tron."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Section(localizedSettingsString("Token Details")) {
                Picker(localizedSettingsString("Chain"), selection: $selectedChain) {
                    ForEach(TokenTrackingChain.allCases) { chain in
                        Text(chain.rawValue).tag(chain)
                    }
                }

                TextField(localizedSettingsString("Symbol"), text: $symbolInput)
                ...
                TextField(
                    selectedChain.contractAddressPrompt,
                    text: $contractInput
                )
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                ...
                TextField(localizedSettingsString("CoinGecko ID (Optional)"), text: $coinGeckoIDInput)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
            }
...
struct DecimalDisplaySettingsView: View {
    ...
    var body: some View {
        Form {
            Section {
                Text(localizedSettingsString("Search native assets and tracked tokens, then adjust how many decimals Spectra shows in portfolio and wallet views."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            ...
            Section(localizedSettingsString("Native Asset Display")) {
                Text(localizedSettingsString("Adjust how many decimals are shown for each chain's native asset. Very small values switch to a threshold marker instead of rounding to zero."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                ...
                if filteredDecimalExamples.isEmpty {
                    Text(localizedSettingsString("No matching native assets."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
                    ForEach(filteredDecimalExamples, id: \.symbol) { example in
                        ...
                        decimalStepperCard(
                            ...
                            fallbackText: Coin.displayMark(for: example.symbol),
                            tint: Coin.displayColor(for: example.symbol),
                            title: example.chainName,
                            subtitle: example.symbol,
                            currentDisplayDecimals: currentDisplayDecimals,
                            supportedDecimals: supportedDecimals,
                            supportedLabel: localizedSettingsString("Asset supports"),
                            onDecrease: {
            ...
            Section(localizedSettingsString("Tracked Token Decimals")) {
                Text(localizedSettingsString("ERC-20 and TRC-20 tokens expose decimals on the contract, and Solana tokens store decimals on the mint account. Manage tracked token decimal support separately from native asset display precision."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                ...
                if filteredTokenDecimalEntries.isEmpty {
                    Text(store.enabledTrackedTokenPreferences.isEmpty ? localizedSettingsString("No tokens are currently enabled for tracking.") : localizedSettingsString("No matching tracked tokens."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
                    ForEach(filteredTokenDecimalEntries, id: \.id) { entry in
                        ...
                        decimalStepperCard(
                            ...
                            fallbackText: String(entry.symbol.prefix(2)).uppercased(),
                            tint: decimalTokenTint(for: entry.chain),
                            title: entry.name,
                            subtitle: "\(entry.chain.rawValue) · \(entry.symbol)",
                            currentDisplayDecimals: currentDisplayDecimals,
                            supportedDecimals: supportedDecimals,
                            supportedLabel: localizedSettingsString("Token supports"),
                            detailText: entry.contractAddress,
                            onDecrease: {
    ...
    private var filteredDecimalExamples: [(symbol: String, chainName: String)] {
        ...
        return decimalExamples.filter { example in
            [example.symbol, example.chainName]
                .joined(separator: " ")
                .lowercased()
                .contains(query)
        }
    ...
    private func decimalStepperCard(
        ...
        title: String,
        subtitle: String,
        currentDisplayDecimals: Int,
        supportedDecimals: Int,
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top, spacing: 12) {
                ...
                HStack(spacing: 10) {
                    ...
                    .buttonStyle(.plain)
                    .disabled(currentDisplayDecimals <= 0)

                    Text("\(currentDisplayDecimals)")
                    ...
                    .buttonStyle(.plain)
                    .disabled(currentDisplayDecimals >= supportedDecimals)
                }
                .font(.title3)
            }
            ...
            HStack {
                Text(supportedLabel)
                Spacer()
                Text(localizedSettingsFormat("%lld decimals", supportedDecimals))
                    .foregroundStyle(.secondary)
            }
            .font(.caption)
        }
...
struct LogsView: View {
    ...
    private enum LogLevelFilter: CaseIterable, Identifiable {
        ...
        var title: String {
            ...
            case .all:
                return localizedSettingsString("All")
            case .debug:
                return localizedSettingsString("Debug")
            case .info:
                return localizedSettingsString("Info")
            case .warning:
                return localizedSettingsString("Warning")
            case .error:
                return localizedSettingsString("Error")
            }
        }
    }
    ...
    private func rebuildLogPresentation() {
        ...
        cachedFilteredLogs = diagnosticsState.operationalLogs.filter { event in
            ...
            case .all:
                levelMatches = true
            case .debug:
                levelMatches = event.level == .debug
            case .info:
                levelMatches = event.level == .info
            case .warning:
                levelMatches = event.level == .warning
            case .error:
                levelMatches = event.level == .error
            }
    ...
    var body: some View {
        List {
            Section(localizedSettingsString("Status")) {
                Text(store.pendingTransactionRefreshStatusText ?? localizedSettingsString("No refresh status yet"))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Text(store.networkSyncStatusText)
                    .font(.caption)
            ...
            Section(localizedSettingsString("Filters")) {
                ...
                Picker(localizedSettingsString("Category"), selection: $selectedCategoryFilter) {
                    ForEach(availableCategories, id: \.self) { category in
                        Text(category == allCategoryFilter ? localizedSettingsString("All") : category).tag(category)
                    }
                }
            }
            ...
            if filteredLogs.isEmpty {
                Section(localizedSettingsString("Events")) {
                    Text(localizedSettingsString("No operational events yet."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            } else {
                Section(localizedSettingsString("Events")) {
                    ForEach(filteredLogs) { event in
                        VStack(alignment: .leading, spacing: 6) {
                            HStack(spacing: 8) {
                                Image(systemName: iconName(for: event.level))
                            ...
                            if let source = event.source, !source.isEmpty {
                                Text(localizedSettingsFormat("source: %@", source))
                                    .font(.caption.monospaced())
                                    .foregroundStyle(.secondary)
                            }

                            if let chainName = event.chainName, !chainName.isEmpty {
                                Text(localizedSettingsFormat("chain: %@", chainName))
                                    .font(.caption.monospaced())
                                    .foregroundStyle(.secondary)
                            }

                            if let walletID = event.walletID {
                                Text(localizedSettingsFormat("wallet: %@", walletID.uuidString))
                                    .font(.caption.monospaced())
                                    .foregroundStyle(.secondary)
                                    .textSelection(.enabled)
                            }
...
struct ResetWalletWarningView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                Section {
                    Text(localizedSettingsString("Choose which categories to remove from this device. Selected items are deleted locally and some options also clear secure keychain data."))
                        .font(.body)
                    Text(localizedSettingsString("You must have your seed phrase backed up. Without it, you cannot recover your funds after reset."))
                ...
                Section(localizedSettingsString("Selected Reset Summary")) {
                    if selectedScopes.contains(.walletsAndSecrets) {
                        Label(localizedSettingsString("Imported wallets, watched addresses, and secure seed material"), systemImage: "wallet.pass")
                    }
                    if selectedScopes.contains(.historyAndCache) {
                        Label(localizedSettingsString("Transaction history, chain snapshots, diagnostics, and network caches"), systemImage: "clock.arrow.circlepath")
                    }
                    if selectedScopes.contains(.alertsAndContacts) {
                        Label(localizedSettingsString("Price alerts, notification rules, and address book recipients"), systemImage: "bell.slash")
                    }
                    if selectedScopes.contains(.settingsAndEndpoints) {
                        Label(localizedSettingsString("Tracked tokens, API keys, endpoint settings, preferences, and custom icons"), systemImage: "slider.horizontal.3")
                    }
                    ...
                    if selectedScopes.contains(.providerState) {
                        Label(localizedSettingsString("Provider selections, reliability memory, and low-level network state"), systemImage: "network")
                    }
                    if selectedScopes.isEmpty {
                        Text(localizedSettingsString("Select at least one category to enable reset."))
                            .foregroundStyle(.secondary)
                    }
                }
            ...
            .navigationTitle(localizedSettingsString("Reset Wallet"))
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(localizedSettingsString("Cancel")) {
                        dismiss()
...
struct TokenIconSettingsView: View {
    ...
    var body: some View {
        Form {
            Section {
                ForEach(filteredSettings) { setting in
                    TokenIconCustomizationRow(setting: setting)
                }
            } header: {
        ...
        .navigationTitle(localizedSettingsString("Icon Styles"))
        .searchable(text: $searchText, prompt: localizedSettingsString("Search icons"))
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
