...
struct SendView: View {
    ...
    @State private var qrScannerErrorMessage: String? init(store: WalletStore) {
        self.store = store
    ...
    private var selectedNetworkSendCoin: Coin? {
        store.availableSendCoins(for: flowState.sendWalletID)..first(where: { $0.holdingKey == flowState.sendHoldingKey })
    }
    @ViewBuilder
    private var sendStatusSections: some View {
        ...
        if let sendVerificationNotice = flowState.sendVerificationNotice {
            sendDetailCard(title: "Verification") {
                Text(sendVerificationNotice)..font(.caption).foregroundStyle(flowState.sendVerificationNoticeIsWarning ? .red : .orange)
            }}
        ...
        optionalSendingSection(sendState.isSendingBitcoin, "Broadcasting Bitcoin transaction...")
        optionalSendingSection(sendState.isSendingBitcoinCash, "Broadcasting Bitcoin Cash transaction...")
        optionalSendingSection(sendState.isSendingBitcoinSV, "Broadcasting Bitcoin SV transaction...")
        optionalSendingSection(sendState.isSendingLitecoin, "Broadcasting Litecoin transaction...")
        optionalSendingSection(sendState.isSendingEthereum, "Broadcasting \(store.selectedSendCoin?.chainName ?? "EVM") transaction...")
        optionalSendingSection(sendState.isSendingDogecoin, "Broadcasting Dogecoin transaction...")
        optionalSendingSection(sendState.isSendingTron, "Broadcasting Tron transaction...")
        optionalSendingSection(sendState.isSendingSolana, "Broadcasting Solana transaction...")
        optionalSendingSection(sendState.isSendingXRP, "Broadcasting XRP transaction...")
        optionalSendingSection(sendState.isSendingStellar, "Broadcasting Stellar transaction...")
        optionalSendingSection(sendState.isSendingMonero, "Broadcasting Monero transaction...")
        optionalSendingSection(sendState.isSendingCardano, "Broadcasting Cardano transaction...")
        optionalSendingSection(sendState.isSendingSui, "Broadcasting Sui transaction...")
        optionalSendingSection(sendState.isSendingAptos, "Broadcasting Aptos transaction...")
        optionalSendingSection(sendState.isSendingTON, "Broadcasting TON transaction...")
        optionalSendingSection(sendState.isSendingICP, "Broadcasting Internet Computer transaction...")
        optionalSendingSection(sendState.isSendingNear, "Broadcasting NEAR transaction...")
        optionalSendingSection(sendState.isSendingPolkadot, "Broadcasting Polkadot transaction...")
    }
    @ViewBuilder
    ...
    private func networkSendSections(selectedCoin: Coin?) -> some View {
        ...
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Tron", isPreparing: runtimeState.isPreparingTronSend, fee: store.tronSendPreview.map { ($0.estimatedNetworkFeeTRX, "TRX", "%.6f") }, footer: "Spectra signs and broadcasts Tron transfers in-app, including TRX and TRC-20 USDT.", extraCaption: selectedCoin?.symbol == "USDT" ? "USDT on Tron uses TRX for network fees. Keep a TRX balance for gas." : nil
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "XRP Ledger", isPreparing: runtimeState.isPreparingXRPSend, fee: store.xrpSendPreview.map { ($0.estimatedNetworkFeeXRP, "XRP", "%.6f") }, footer: "Spectra signs and broadcasts XRP transfers in-app.", extraLines: store.xrpSendPreview.map { p in
                [p.sequence > 0 ? "Sequence: \(p.sequence)" : nil, p.lastLedgerSequence > 0 ? "Last Ledger Sequence: \(p.lastLedgerSequence)" : nil].compactMap { $0 }} ?? []
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Solana", isPreparing: runtimeState.isPreparingSolanaSend, fee: store.solanaSendPreview.map { ($0.estimatedNetworkFeeSOL, "SOL", "%.6f") }, footer: "Spectra signs and broadcasts Solana transfers in-app, including SOL and supported SPL assets.", extraCaption: selectedCoin?.symbol != "SOL" ? "Token transfers on Solana still use SOL for network fees." : nil
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Cardano", isPreparing: runtimeState.isPreparingCardanoSend, fee: store.cardanoSendPreview.map { ($0.estimatedNetworkFeeADA, "ADA", "%.6f") }, footer: "Spectra signs and broadcasts ADA transfers in-app.", extraLines: store.cardanoSendPreview.map { p in p.ttlSlot > 0 ? ["TTL Slot: \(p.ttlSlot)"] : [] } ?? []
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Monero", isPreparing: runtimeState.isPreparingMoneroSend, fee: store.moneroSendPreview.map { ($0.estimatedNetworkFeeXMR, "XMR", "%.6f") }, footer: "Spectra prepares Monero sends in-app using the configured backend fee quote.", extraLines: store.moneroSendPreview.map { ["Priority: \($0.priorityLabel)"] } ?? []
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "NEAR", isPreparing: runtimeState.isPreparingNearSend, fee: store.nearSendPreview.map { ($0.estimatedNetworkFeeNEAR, "NEAR", "%.6f") }, footer: "Spectra signs and broadcasts NEAR transfers in-app."
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Polkadot", isPreparing: runtimeState.isPreparingPolkadotSend, fee: store.polkadotSendPreview.map { ($0.estimatedNetworkFeeDOT, "DOT", "%.6f") }, footer: "Spectra signs and broadcasts Polkadot transfers in-app."
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Stellar", isPreparing: runtimeState.isPreparingStellarSend, fee: store.stellarSendPreview.map { ($0.estimatedNetworkFeeXLM, "XLM", "%.7f") }, footer: "Spectra signs and broadcasts Stellar payments in-app.", extraLines: store.stellarSendPreview.map { p in p.sequence > 0 ? ["Sequence: \(p.sequence)"] : [] } ?? []
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Internet Computer", isPreparing: runtimeState.isPreparingICPSend, fee: store.icpSendPreview.map { ($0.estimatedNetworkFeeICP, "ICP", "%.8f") }, footer: "Spectra signs and broadcasts ICP transfers in-app."
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Sui", isPreparing: runtimeState.isPreparingSuiSend, fee: store.suiSendPreview.map { ($0.estimatedNetworkFeeSUI, "SUI", "%.6f") }, footer: "Spectra signs and broadcasts Sui transfers in-app.", extraLines: store.suiSendPreview.map { ["Gas Budget: \($0.gasBudgetMist) MIST", "Reference Gas Price: \($0.referenceGasPrice)"] } ?? []
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "Aptos", isPreparing: runtimeState.isPreparingAptosSend, fee: store.aptosSendPreview.map { ($0.estimatedNetworkFeeAPT, "APT", "%.6f") }, footer: "Spectra signs and broadcasts Aptos transfers in-app.", extraLines: store.aptosSendPreview.map { ["Max Gas Amount: \($0.maxGasAmount)", "Gas Unit Price: \($0.gasUnitPriceOctas) octas"] } ?? []
        )
        simpleSendNetworkSection(
            for: selectedCoin, chainName: "TON", isPreparing: runtimeState.isPreparingTONSend, fee: store.tonSendPreview.map { ($0.estimatedNetworkFeeTON, "TON", "%.6f") }, footer: "Spectra signs and broadcasts TON transfers in-app.", extraLines: store.tonSendPreview.map { ["Sequence Number: \($0.sequenceNumber)"] } ?? []
        )
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 18) {
                    SendPrimarySectionsView(
                        store: store, flowState: flowState, selectedAddressBookEntryID: $selectedAddressBookEntryID, isShowingQRScanner: $isShowingQRScanner, qrScannerErrorMessage: $qrScannerErrorMessage
                    )
                ...
                if let qrScannerErrorMessage { Text(verbatim: qrScannerErrorMessage) }}..onChange(of: flowState.sendHoldingKey) { _, _ in
...
private struct SendPrimarySectionsView: View {
    ...
    @Binding var qrScannerErrorMessage: String? private struct Presentation {
        let sendWallets: [ImportedWallet]
        let selectedWallet: ImportedWallet? let availableSendCoins: [Coin]
        let selectedCoin: Coin? let selectedCoinAmountText: String? let selectedCoinApproximateFiatText: String? let addressBookEntries: [AddressBookEntry]
    }
    private var presentation: Presentation {
        ...
        let selectedCoinApproximateFiatText: String? if let selectedCoin, !sendAmount.isZero { selectedCoinApproximateFiatText = store.formattedFiatAmount(fromNative: sendAmount, symbol: selectedCoin.symbol) } else { selectedCoinApproximateFiatText = nil }
        return Presentation(
        ...
        sendDetailCard(title: "Wallet & Asset") {
            VStack(alignment: .leading, spacing: 12) {
                Picker("Wallet", selection: store.sendWalletIDBinding) {
                    ForEach(presentation.sendWallets) { wallet in Text(wallet.name).tag(wallet.id.uuidString) }}..onChange(of: flowState.sendWalletID) { _, _ in
    ...
    private var recipientSection: some View {
        sendDetailCard(title: "Recipient") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let sendDestinationRiskWarning = flowState.sendDestinationRiskWarning { Text(sendDestinationRiskWarning)..font(.caption).foregroundStyle(.orange) }
                if let sendDestinationInfoMessage = flowState.sendDestinationInfoMessage { Text(sendDestinationInfoMessage)..font(.caption).foregroundStyle(.secondary) }}}}
