    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text(settingsLocalizedString("Contract / Mint"))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            ...
            if !entry.isBuiltIn {
                Stepper(
                    settingsLocalizedFormat("Supports: %lld decimals", entry.decimals),
                    value: Binding(
                        get: { entry.decimals },
                        set: updateDecimals
                    ),
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if selectedStyle == .customPhoto || hasCustomPhoto {
                ...
                if !hasCustomPhoto {
                    Text(settingsLocalizedString("Select a photo from your library to use as this token icon."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
