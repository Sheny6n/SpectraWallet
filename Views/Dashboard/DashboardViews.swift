...
struct DashboardView: View {
    ...
    @State private var selectedWalletID: UUID? @State private var walletPageIndex: Int = 0
    @State private var selectedAssetGroup: DashboardAssetGroup? @State private var isShowingAddWalletPage: Bool = false
    init(store: WalletStore) {
        ...
        _refreshSignal = StateObject(
            wrappedValue: ViewRefreshSignal([ store.dashboardState.objectWillChange.asVoidSignal(), store.portfolioState.objectWillChange.asVoidSignal(), store.flowState.objectWillChange.asVoidSignal(), store.$hideBalances.asVoidSignal(), store.$quoteRefreshError.asVoidSignal(), store.$fiatRatesRefreshError.asVoidSignal()
            ])
        )
    }
    private var deleteWalletMessage: String {
        guard let pendingWallet = flowState.walletPendingDeletion else { return "" }
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                get: { flowState.isShowingWalletImporter && flowState.editingWalletID == nil }, set: { isPresented in
                ...
                if let selectedWallet { WalletDetailView(store: store, wallet: selectedWallet) }}.navigationDestination(item: $selectedAssetGroup) { assetGroup in AssetGroupDetailView(store: store, assetGroup: assetGroup) }.navigationDestination(isPresented: Binding(get: { flowState.isShowingSendSheet }, set: { flowState.isShowingSendSheet = $0 })) {
                SendView(store: store)
            }.navigationDestination(isPresented: Binding(get: { flowState.isShowingReceiveSheet }, set: { flowState.isShowingReceiveSheet = $0 })) {
                ReceiveView(store: store)
            }..alert("Delete Wallet?", isPresented: Binding(
                get: { flowState.walletPendingDeletion != nil }, set: { isPresented in
                    if !isPresented { flowState.walletPendingDeletion = nil }}
            )) {
    ...
    private var portfolioHeader: some View {
        HStack {
            ...
            VStack(alignment: .trailing, spacing: 8) {
                ...
                Text(localizedFormat("%lld in total", portfolioState.includedPortfolioWallets.count))..font(.caption2).foregroundStyle(Color.primary.opacity(0.72))
    ...
    private var visiblePortfolio: [DashboardAssetGroup] { dashboardState.assetGroups }
    ...
    private var dashboardCardCountText: String { dashboardPage == .assets ? "\(visiblePortfolio.count)" : "\(portfolioState.wallets.count)" }
...
struct DashboardAssetChainEntry: Identifiable {
    let coin: Coin
    let valueUSD: Double? var id: String {
...
struct DashboardAssetGroup: Identifiable, Hashable {
    ...
    let totalValueUSD: Double? let chainEntries: [DashboardAssetChainEntry]
    let isPinned: Bool
...
struct DashboardPinOption: Identifiable {
    ...
    let name: String
    let subtitle: String
    let assetIdentifier: String? let mark: String
    let color: Color
...
struct AssetGroupDetailView: View {
    ...
    private var supportedTokenEntries: [TokenPreferenceEntry] { dashboardState.supportedTokenEntriesBySymbol[assetGroup.symbol.uppercased()] ?? [] }
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 14) {
