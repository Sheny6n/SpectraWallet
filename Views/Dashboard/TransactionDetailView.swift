...
struct HistoryDetailView: View {
    ...
    private var ownedAddresses: Set<String> {
        liveOwnedAddresses
    }
    ...
    private var fromAddressText: String? {
        ...
        if normalizedAddress(counterparty) != normalizedAddress(walletSideAddress) {
            return counterparty
        }
    ...
    private var toAddressText: String? {
        if displayedTransaction.kind == .send {
            let counterparty = nonEmptyAddress(displayedTransaction.addressPreviewText)
            if normalizedAddress(counterparty) != normalizedAddress(fromAddressText) {
                return counterparty
            }
            return nil
        }
    ...
    private var walletSideAddress: String? {
        if let sourceAddress = nonEmptyAddress(displayedTransaction.sourceAddress),
           isOwnedAddress(sourceAddress) {
            return sourceAddress
        }

        if let previewAddress = nonEmptyAddress(displayedTransaction.addressPreviewText),
           isOwnedAddress(previewAddress) {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    detailCard(title: "Overview") {
                        ...
                        if let failureReason = displayedTransaction.failureReason {
                            detailRow(label: "Failure", value: failureReason)
                        }
                    }
                    
                    if displayedTransaction.chainName == "Ethereum",
                       displayedTransaction.kind == .send,
                       displayedTransaction.status == .pending {
                        detailCard(title: "Ethereum Mempool Actions") {
                            if store.isPreparingEthereumReplacementContext {
                                ...
                                Button {
                                    Task {
                                        ethereumReplacementMessage = await store.openEthereumReplacementComposer(
                                            for: displayedTransaction.id,
                                            cancel: false
                                        )
                                    }
                                } label: {
                                    Text("Speed Up This Transaction")
                                        .font(.headline)
                                ...
                                Text("This opens the Send composer with the same nonce and higher fee defaults so you can safely speed up or cancel the pending transaction.")
                                    .font(.caption)
                                    .foregroundStyle(Color.primary.opacity(0.72))
                            }

                            if let ethereumReplacementMessage {
                                Text(ethereumReplacementMessage)
                                    .font(.caption)
                                    .foregroundStyle(Color.primary.opacity(0.72))
                            }
                        }
                    ...
                    detailCard(title: "Addresses") {
                        ...
                        if let toAddressText {
                            addressBlock(label: "To", value: toAddressText, isMine: isOwnedAddress(toAddressText))
                        }
                    }
    ...
    private var statusChip: some View {
        Text(displayedTransaction.statusText)
            .font(.caption.bold())
            .foregroundStyle(Color.primary)
            .padding(.horizontal, 10)
            .padding(.vertical, 6)
            .background(displayedTransaction.statusColor.opacity(0.32), in: Capsule())
            .overlay(
                Capsule()
                    .stroke(displayedTransaction.statusColor.opacity(0.45), lineWidth: 1)
            )
    }
            ...
            HStack(spacing: 8) {
                ...
                if isMine {
                    Text(NSLocalizedString("Mine", comment: ""))
                        ...
                        .overlay(
                            Capsule()
                                .stroke(Color.mint.opacity(0.35), lineWidth: 1)
                        )
                }
            }
    ...
    private func normalizedAddress(_ value: String?) -> String? {
        ...
        case "Ethereum", "Arbitrum", "Optimism", "BNB Chain", "Avalanche", "Hyperliquid":
