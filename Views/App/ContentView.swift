...
private struct SpectraInputFieldChrome: ViewModifier {
    ...
    let borderColor: Color? private var resolvedBackground: Color { colorScheme == .light ? Color.black.opacity(0.045) : Color.white.opacity(0.08) }
...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
            MainTabView(store: store)..blur(radius: runtimeState.isAppLocked ? 8 : 0).disabled(runtimeState.isAppLocked)
            if runtimeState.isAppLocked {
                VStack(spacing: 14) {
                    ...
                    if let appLockError = runtimeState.appLockError { Text(appLockError)..font(.caption).foregroundStyle(.red) }
                    Button {
                        Task {
