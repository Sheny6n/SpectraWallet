...
enum StaticContentCatalog {
    ...
    private static let rustStaticJSONResources: Set<String> = [
        "TokenVisualRegistry",
        "ChainVisualRegistry",
        "BuyCryptoProviders",
        "BuiltInTokenRegistry"
    ]
    private static let rustStaticTextResources: Set<String> = [
    ...
    static func loadResource<T: Decodable>(_ resourceName: String, as type: T.Type) -> T? {
        ...
        if rustStaticJSONResources.contains(resourceName),
           let data = try? WalletRustAppCoreBridge.staticDocumentData(named: resourceName),
           let decodedValue = try? decoder.decode(type, from: data) {
            decodedResourceCache[cacheKey] = decodedValue
            return decodedValue
        }
    ...
    static func loadTextResource(_ resourceName: String, extension fileExtension: String = "txt") -> String? {
        ...
        if fileExtension == "txt",
           rustStaticTextResources.contains(resourceName),
           let text = try? WalletRustAppCoreBridge.staticText(named: resourceName) {
