...
enum BitcoinCashProvider {
    ...
    struct ActorForthEnvelope<Payload: Decodable>: Decodable {
        let status: String? let message: String? let data: Payload? }
    struct ActorForthAddressDetails: Decodable {
        let balanceSat: Int64? let txApperances: Int? let transactions: [String]? enum CodingKeys: String, CodingKey {
            case balanceSat
            case txApperances
            case transactions
        }}
    struct ActorForthUTXOPayload: Decodable {
        struct Entry: Decodable {
            let txid: String? let vout: Int? let satoshis: UInt64? }
        let utxos: [Entry]? }
    struct ActorForthTransactionPayload: Decodable {
        struct Input: Decodable {
            let legacyAddress: String? let cashAddress: String? let valueSat: Int64? enum CodingKeys: String, CodingKey {
                case legacyAddress
                case cashAddress
                case valueSat
            }}
        struct Output: Decodable {
            let legacyAddress: String? let cashAddress: String? let value: String? let valueSat: Int64? enum CodingKeys: String, CodingKey {
                case legacyAddress
                case cashAddress
                case value
                case valueSat
            }}
        let txid: String? let confirmations: Int? let blockheight: Int? let time: TimeInterval? let vin: [Input]? let vout: [Output]? }
