...
enum AptosProvider {
    ...
    struct CoinStoreResource: Decodable {
        let data: CoinStoreData? struct CoinStoreData: Decodable {
            let coin: CoinValue? }
        struct CoinValue: Decodable {
            let value: String? }}
    struct AccountResource: Decodable {
        let type: String? let data: CoinStoreResource.CoinStoreData? }
    struct TransactionItem: Decodable {
        let type: String? let hash: String? let success: Bool? let sender: String? let timestamp: String? let payload: Payload? struct Payload: Decodable {
            let function: String? let arguments: [String]? }}
    ...
    struct TransactionLookupResponse: Decodable {
        let hash: String? let success: Bool? let vmStatus: String? enum CodingKeys: String, CodingKey {
            case hash
            case success
