...
enum TONProvider {
    ...
    struct WalletInformationEnvelope: Decodable {
        let ok: Bool? let result: WalletInformationResult? let error: String? }
    struct WalletInformationResult: Decodable {
        let balance: String? let seqno: UInt32? }
    struct TransactionsEnvelope: Decodable {
        let ok: Bool? let result: [TransactionEntry]? let error: String? }
    struct JettonWalletsEnvelope: Decodable {
        let jettonWallets: [JettonWalletEntry]? enum CodingKeys: String, CodingKey {
            case jettonWallets = "jetton_wallets"
        }}
    struct JettonWalletEntry: Decodable {
        let balance: String? let address: String? let owner: AddressEnvelope? let jetton: AddressEnvelope? struct AddressEnvelope: Decodable {
            let address: String? }}
    struct TransactionEntry: Decodable {
        let utime: Int? let transactionID: TransactionID? let inMsg: Message? let outMsgs: [Message]? enum CodingKeys: String, CodingKey {
            case utime
    ...
    struct Message: Decodable {
        let source: String? let destination: String? let value: String? }
    struct SendBocEnvelope: Decodable {
        let ok: Bool? let result: SendBocResult? let error: String? }
