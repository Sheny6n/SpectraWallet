...
enum UTXOAddressCodec {
    ...
    static func base58CheckDecode(_ string: String) -> Data? {
        ...
        for (index, character) in alphabet.enumerated() { indexes[character] = index }
        var bytes: [UInt8] = [0]
    ...
    static func base58CheckEncode(_ payload: Data) -> String {
        ...
        for byte in full where byte == 0 { encoded.append("1") }
        return String(encoded.reversed())
    }
    static func hash160(_ data: Data) -> Data { RIPEMD160.hash(data: Data(SHA256.hash(data: data))) }
    static func legacyScriptPubKey(for address: String, p2pkhVersions: Set<UInt8>, p2shVersions: Set<UInt8>) -> Data? {
        guard let decoded = base58CheckDecode(address), !decoded.isEmpty else { return nil }
        let prefix = decoded[0]
        let hash160 = decoded.dropFirst()
        guard hash160.count == 20 else { return nil }
        if p2pkhVersions.contains(prefix) { return Data([0x76, 0xa9, 0x14]) + hash160 + Data([0x88, 0xac]) }
        if p2shVersions.contains(prefix) { return Data([0xa9, 0x14]) + hash160 + Data([0x87]) }
        return nil
    }
    static func legacyP2PKHAddress(privateKeyData: Data, version: UInt8) throws -> String {
    ...
    static func nestedSegWitP2SHAddress(privateKeyData: Data, scriptVersion: UInt8) throws -> String {
        let publicKey = try compressedSecp256k1PublicKey(privateKeyData: privateKeyData)
    ...
    static func segWitAddress(privateKeyData: Data, hrp: String, witnessVersion: UInt8 = 0, encoding: SegWitAddressEncoding = .bech32) throws -> String {
        let compressedPublicKey = try compressedSecp256k1PublicKey(privateKeyData: privateKeyData)
        let program: Data
        switch witnessVersion {
        case 0: program = hash160(compressedPublicKey)
        case 1: guard compressedPublicKey.count == 33 else { throw UTXOAddressCodecError.unsupportedWitnessVersion }
            program = compressedPublicKey.dropFirst()
        default: throw UTXOAddressCodecError.unsupportedWitnessVersion
        }
        guard let convertedProgram = convertBits(
            Array(program), fromBits: 8, toBits: 5, pad: true
        ) else {
            throw UTXOAddressCodecError.invalidWitnessProgram
        }
        let data = [witnessVersion] + convertedProgram
        return try bech32Encode(
            hrp: hrp.lowercased(), data: data, encoding: encoding
        )
    }
    private static func bech32Encode(hrp: String, data: [UInt8], encoding: SegWitAddressEncoding) throws -> String {
        ...
        guard combined.allSatisfy({ Int($0) < alphabet.count }) else { throw UTXOAddressCodecError.invalidWitnessProgram }
        let payload = combined.map { String(alphabet[Int($0)]) }.joined()
        return "\(hrp)1\(payload)"
    }
    private static func createBech32Checksum(hrp: String, data: [UInt8], encoding: SegWitAddressEncoding) -> [UInt8] {
        ...
        return (0..<6).map { index in UInt8((polymod >> (5 * (5 - index))) & 31) }}
    ...
    private static func bech32Polymod(_ values: [UInt8]) -> UInt32 {
        ...
        for value in values {
            ...
            for (index, generator) in generators.enumerated() where ((top >> index) & 1) != 0 { checksum ^= generator }}
        return checksum
    }
    private static func convertBits(_ data: [UInt8], fromBits: Int, toBits: Int, pad: Bool) -> [UInt8]? {
        ...
        if pad {
            if bits > 0 { result.append(UInt8((accumulator << (toBits - bits)) & maxValue)) }
        } else if bits >= fromBits || ((accumulator << (toBits - bits)) & maxValue) != 0 { return nil }
        return result
    }
    private static func compressedSecp256k1PublicKey(privateKeyData: Data) throws -> Data {
        guard privateKeyData.count == 32 else { throw UTXOAddressCodecError.invalidPrivateKey }
        ...
        let response = try WalletRustDerivationBridge.deriveFromPrivateKey(
            chain: .bitcoin, network: .mainnet, privateKeyHex: privateKeyHex
        )
        guard let publicKeyHex = response.publicKeyHex, let publicKeyData = Data(hexEncoded: publicKeyHex), publicKeyData.count == 33 else { throw UTXOAddressCodecError.invalidPrivateKey }
...
private extension UInt32 {
    func rotateLeft(by amount: UInt32) -> UInt32 { (self << amount) | (self >> (32 - amount)) }
}
