...
enum BitcoinSVWalletEngine {
    ...
    private static func mapRustUTXOPlannerError(_ error: Error) -> BitcoinSVWalletEngineError {
        switch error.localizedDescription {
        case "utxo.insufficientFunds":
            return .insufficientFunds
        case "utxo.amountBelowDustThreshold":
            return .policyViolation("Amount is below Bitcoin SV dust threshold.")
        case "utxo.feeBelowRelayPolicy":
            return .policyViolation("Bitcoin SV fee rate is below standard relay policy.")
        case "utxo.transactionTooLarge":
            return .policyViolation("Bitcoin SV transaction is too large for standard relay policy.")
        case "utxo.changeBelowDustThreshold":
            return .policyViolation("Calculated Bitcoin SV change is below dust threshold.")
        default:
