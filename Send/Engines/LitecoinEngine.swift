...
enum LitecoinWalletEngine {
    ...
    static func estimateSendPreview(
        ...
        let preview = try rustPreviewPlan(from: utxos, feeRateSatVb: rate)
    ...
    private static func mapRustUTXOPlannerError(_ error: Error) -> LitecoinWalletEngineError {
        switch error.localizedDescription {
        case "utxo.insufficientFunds":
            return .insufficientFunds
        case "utxo.amountBelowDustThreshold":
            return .policyViolation("Amount is below Litecoin dust threshold.")
        case "utxo.feeBelowRelayPolicy":
            return .policyViolation("Litecoin fee rate is below standard relay policy.")
        case "utxo.transactionTooLarge":
            return .policyViolation("Litecoin transaction is too large for standard relay policy.")
        case "utxo.changeBelowDustThreshold":
            return .policyViolation("Calculated Litecoin change is below dust threshold.")
        default:
