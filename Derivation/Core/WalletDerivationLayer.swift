...
enum WalletDerivationLayer {
    ...
    static func derive(jsonData: Data) throws -> Data { try WalletDerivationEngine.derive(jsonData: jsonData) }
    static func derive(jsonString: String) throws -> String { try WalletDerivationEngine.derive(jsonString: jsonString) }
    static func deriveAddress(seedPhrase: String, chain: SeedDerivationChain, network: WalletDerivationNetwork, derivationPath: String) throws -> String {
        let requestData = try JSONEncoder().encode(
            WalletDerivationLayerJSONRequest(
                chain: chain, network: network, seedPhrase: seedPhrase, derivationPath: derivationPath, curve: WalletDerivationEngine.curve(for: chain), passphrase: nil, iterationCount: nil, hmacKeyString: nil, requestedOutputs: ["address"]
            )
        )
