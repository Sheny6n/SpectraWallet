...
final class WalletRefreshPlannerTests: XCTestCase {
    func testActiveMaintenancePlanRequestsPendingAndPriceRefreshWhenIntervalsElapsed() {
        ...
        let plan = WalletRefreshPlanner.activeMaintenancePlan(
            now: now, lastPendingTransactionRefreshAt: now.addingTimeInterval(-301), lastLivePriceRefreshAt: now.addingTimeInterval(-601), hasPendingTransactionMaintenanceWork: true, shouldRunScheduledPriceRefresh: true, pendingRefreshInterval: 300, priceRefreshInterval: 600
        )
    ...
    func testActiveMaintenancePlanSkipsRefreshesWhenWorkOrScheduleIsMissing() {
        let now = Date()
        let plan = WalletRefreshPlanner.activeMaintenancePlan(
            now: now, lastPendingTransactionRefreshAt: now.addingTimeInterval(-10_000), lastLivePriceRefreshAt: now.addingTimeInterval(-10_000), hasPendingTransactionMaintenanceWork: false, shouldRunScheduledPriceRefresh: false, pendingRefreshInterval: 300, priceRefreshInterval: 600
        )
    ...
    func testChainPlansIncludeForcedPendingAndDegradedChainsWithExpectedHistoryFlags() {
        ...
        let plans = WalletRefreshPlanner.chainPlans(
            for: Set(["Ethereum", "Solana", "Bitcoin"].compactMap(WalletChainID.init)), now: now, forceChainRefresh: false, includeHistoryRefreshes: true, historyRefreshInterval: 300, pendingTransactionMaintenanceChains: Set(["Ethereum"].compactMap(WalletChainID.init)), degradedChains: Set(["Solana"].compactMap(WalletChainID.init)), lastGoodChainSyncByID: [
    ...
    func testHistoryPlansOnlyIncludeChainsWhoseHistoryIsStaleOrMissing() {
        ...
        let chains = WalletRefreshPlanner.historyPlans(
            for: Set(["Ethereum", "Solana", "Bitcoin"].compactMap(WalletChainID.init)), now: now, interval: 300, lastHistoryRefreshAtByChainID: [
                WalletChainID("Ethereum")!: now.addingTimeInterval(-600), WalletChainID("Solana")!: now
            ]
        )
