...
enum ChainBackendRegistry {
    ...
    enum BitcoinRuntimeEndpoints {
        static func esploraBaseURLs(for networkMode: BitcoinNetworkMode) -> [String] { AppEndpointDirectory.bitcoinEsploraBaseURLs(for: networkMode) }
        static func walletStoreDefaultBaseURLs(for networkMode: BitcoinNetworkMode) -> [String] { AppEndpointDirectory.bitcoinWalletStoreDefaultBaseURLs(for: networkMode) }}
    ...
    enum TronRuntimeEndpoints {
        ...
        static let tronGridAccountsBases = AppEndpointDirectory.endpoints(for: [
            "tron.trongrid.accounts.io", "tron.trongrid.accounts.pro", "tron.trongrid.accounts.network"
        ])
        static let tronGridBroadcastBaseURLs = AppEndpointDirectory.endpoints(for: [
            "tron.trongrid.rpc.io", "tron.trongrid.rpc.pro", "tron.trongrid.rpc.network"
        ])
    }
    enum SolanaRuntimeEndpoints {
        static let balanceRPCBaseURLs = AppEndpointDirectory.endpoints(for: [
            "solana.rpc.mainnet", "solana.rpc.ankr", "solana.rpc.publicnode"
        ])
    ...
    enum XRPRuntimeEndpoints {
        static let accountHistoryBases = AppEndpointDirectory.endpoints(for: [
            "xrp.history.xrpscan_api", "xrp.history.xrpscan"
        ])
        static let rpcBaseURLs = AppEndpointDirectory.endpoints(for: [
            "xrp.rpc.s1", "xrp.rpc.s2", "xrp.rpc.cluster"
        ])
    ...
    enum CardanoRuntimeEndpoints {
        static let koiosBaseURLs = AppEndpointDirectory.endpoints(for: [
            "cardano.koios.primary", "cardano.koios.xray", "cardano.koios.happystaking"
        ])
    }
    enum AptosRuntimeEndpoints {
        static let rpcBaseURLs = AppEndpointDirectory.endpoints(for: [
            "aptos.rpc.aptoslabs", "aptos.rpc.blastapi", "aptos.rpc.mainnet"
        ])
        static var rpcURLs: [URL] { rpcBaseURLs.compactMap(URL.init(string:)) }}
    ...
    enum SuiRuntimeEndpoints {
        static let rpcBaseURLs = AppEndpointDirectory.endpoints(for: [
            "sui.rpc.mainnet", "sui.rpc.publicnode", "sui.rpc.blockvision", "sui.rpc.blockpi", "sui.rpc.suiscan"
        ])
        static var rpcURLs: [URL] { rpcBaseURLs.compactMap(URL.init(string:)) }}
    enum NearRuntimeEndpoints {
        static let rpcBaseURLs = AppEndpointDirectory.endpoints(for: [
            "near.rpc.mainnet", "near.rpc.fastnear", "near.rpc.lava"
        ])
    ...
    enum ExplorerRegistry {
        static func transactionURL(for chainName: String, transactionHash: String) -> URL? {
            ...
            if chainName == aptosChainName { return URL(string: "\(baseURL)\(transactionHash)?network=mainnet") }
            return URL(string: baseURL + transactionHash)
        }
        static func transactionLabel(for chainName: String) -> String? { AppEndpointDirectory.transactionExplorerLabel(for: chainName) }}
    ...
    enum EVMExplorerRegistry {
        static func etherscanStyleAPIURL(for chainName: String) -> URL? {
            ...
            case ethereumChainName: AppEndpointDirectory.endpoint("ethereum.explorer.etherscan").replacingOccurrences(of: "/api", with: "/v2/api")
            case bnbChainName: AppEndpointDirectory.endpoint("bnb.explorer.bscscan")
            case avalancheChainName: "https://api.snowtrace.io/api"
            case hyperliquidChainName: AppEndpointDirectory.endpoint("hyperliquid.explorer.api")
            default: nil
            }
            return rawURL.flatMap(URL.init(string:))
        }
        static func blockscoutTokenTransfersURL(for chainName: String, normalizedAddress: String, page: Int, pageSize: Int) -> URL? {
            switch chainName {
            case ethereumChainName: let baseURL = AppEndpointDirectory.endpoint("ethereum.explorer.blockscout")
                return URL(
                    string: "\(baseURL)/api/v2/addresses/\(normalizedAddress)/token-transfers?type=ERC-20&items_count=\(pageSize)&page=\(page)"
                )
            case ethereumClassicChainName: let baseURL = AppEndpointDirectory.endpoint("ethereumclassic.explorer.blockscout")
                return URL(
                    string: "\(baseURL)/api/v2/addresses/\(normalizedAddress)/token-transfers?type=ERC-20&items_count=\(pageSize)&page=\(page)"
                )
            default: return nil
            }}
        static func blockscoutAccountAPIURL(for chainName: String, normalizedAddress: String, action: String, page: Int, pageSize: Int) -> URL? {
            switch chainName {
            case ethereumChainName: guard action == "txlist" else { return nil }
            ...
            case ethereumClassicChainName: guard action == "txlist" else { return nil }
        ...
        static func ethplorerHistoryURL(for chainName: String, normalizedAddress: String, requestedLimit: Int) -> URL? {
        ...
        static func diagnosticProbeEntries(for chainName: String) -> [(String, URL)] {
            switch chainName {
            case ethereumChainName: return [
                    ("Etherscan API", URL(string: "https://api.etherscan.io/api?module=stats&action=ethprice")!), ("Ethplorer API", URL(string: "https://api.ethplorer.io/getAddressInfo/0x0000000000000000000000000000000000000000?apiKey=freekey")!)
                ]
            case bnbChainName: return [
                    ("BscScan API", URL(string: "https://api.bscscan.com/api?module=stats&action=bnbprice")!)
                ]
            default: return []
            }}
        static func supplementalEndpointCatalogEntries(for chainName: String) -> [String] { AppEndpointDirectory.explorerSupplementalEndpoints(for: chainName) }}
