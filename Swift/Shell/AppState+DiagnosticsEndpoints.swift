...
//   * URLSession-based HTTP probes (still Swift-owned pending Phase D)
...
extension AppState {
    ...
    private func runBitcoinHistoryDiagnosticsInner(for wallet: ImportedWallet) async {
        ...
        do {
            ...
            if identifier.isEmpty {
                ...
                bitcoinHistoryDiagnosticsByWallet[wallet.id] = BitcoinHistoryDiagnostics(walletID: wallet.id, identifier: identifier, sourceUsed: page.sourceUsed, transactionCount: page.snapshots.count, nextCursor: page.nextCursor, error: nil)
            }
        } catch {
    ...
    func runPolkadotEndpointReachabilityDiagnostics() async {
        ...
        for (endpoint, probeURL) in PolkadotBalanceService.diagnosticsChecks() {
            if PolkadotBalanceService.sidecarEndpointCatalog().contains(endpoint) {
                guard let url = URL(string: probeURL) else { results.append(BitcoinEndpointHealthResult(endpoint: endpoint, reachable: false, statusCode: nil, detail: "Invalid URL")); continue }
                do {
    ...
    private func probeJSONRPC(endpoint: String, urlString: String, rpcMethod: String) async -> BitcoinEndpointHealthResult {
        ...
        do {
            let payload = try JSONSerialization.data(withJSONObject: ["jsonrpc": "2.0", "id": "spectra-health", "method": rpcMethod, "params": []])
            ...
            let outcome = diagnosticsParseJsonrpcProbe(statusCode: statusCode.map { Int32($0) }, bodyUtf8: String(data: data, encoding: .utf8) ?? "")
            return BitcoinEndpointHealthResult(endpoint: endpoint, reachable: outcome.reachable, statusCode: statusCode, detail: outcome.detail)
        } catch {
            return BitcoinEndpointHealthResult(endpoint: endpoint, reachable: false, statusCode: nil, detail: error.localizedDescription)
        }
    }
    ...
    func runSimpleEndpointReachabilityDiagnostics(checks: [(endpoint: String, probeURL: String)], profile: NetworkRetryProfile, setResults: ([BitcoinEndpointHealthResult]) -> Void, markUpdated: () -> Void) async {
        var results: [BitcoinEndpointHealthResult] = []
        for check in checks {
    ...
    func probeHTTP(_ url: URL, profile: NetworkRetryProfile = .diagnostics) async -> (reachable: Bool, statusCode: Int?, detail: String) {
        do {
            return try await withTimeout(seconds: 10) {
                ...
                let statusCode = (response as? HTTPURLResponse)?.statusCode
                if let statusCode { return ((200 ..< 300).contains(statusCode), statusCode, "HTTP \(statusCode)") }
    ...
    func probeEthereumRPC(_ url: URL) async -> (reachable: Bool, statusCode: Int?, detail: String) {
        do {
            return try await withTimeout(seconds: 10) {
                ...
                request.setValue("application/json", forHTTPHeaderField: "Content-Type")
                request.httpBody = #"{"jsonrpc":"2.0","id":1,"method":"eth_chainId","params":[]}"#.data(using: .utf8)
                let (data, response) = try await ProviderHTTP.data(for: request, profile: .diagnostics)
                let statusCode = (response as? HTTPURLResponse)?.statusCode
    ...
    private func runUTXOStyleHistoryDiagnostics(chainId: UInt32, isRunningKP: ReferenceWritableKeyPath<AppState, Bool>, chainName: String, resolveAddress: @escaping (ImportedWallet) -> String?, diagsKP: ReferenceWritableKeyPath<AppState, [String: BitcoinHistoryDiagnostics]>, tsKP: ReferenceWritableKeyPath<AppState, Date?>) async {
        await runAddressHistoryDiagnosticsForAllWallets(isRunning: { self[keyPath: isRunningKP] }, setRunning: { self[keyPath: isRunningKP] = $0 }, chainName: chainName, resolveAddress: resolveAddress, fetchDiagnostics: { address in
            ...
            return BitcoinHistoryDiagnostics(walletID: "", identifier: address, sourceUsed: "rust", transactionCount: count, nextCursor: nil, error: nil)
        }, storeDiagnostics: { walletID, d in self[keyPath: diagsKP][walletID] = BitcoinHistoryDiagnostics(walletID: walletID, identifier: d.identifier, sourceUsed: d.sourceUsed, transactionCount: Int(d.transactionCount), nextCursor: d.nextCursor, error: d.error) }, markUpdated: { self[keyPath: tsKP] = Date() })
    }
