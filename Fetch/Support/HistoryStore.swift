...
enum HistoryDatabaseStoreError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable: return AppLocalization.string("The history database is unavailable.")
        case .sqlite(let message): let format = AppLocalization.string("SQLite error: %@")
...
final class HistoryDatabaseStore {
    ...
    private let databaseHandle: OpaquePointer? private let encoder = JSONEncoder()
    private let decoder = JSONDecoder()
    private let queue = DispatchQueue(label: "app.spectra.history-database")
    private init() {
        var configuredHandle: OpaquePointer? do {
            ...
            if let configuredHandle {
                try Self.execute(sql: "PRAGMA journal_mode=WAL", on: configuredHandle)
                try Self.execute(
                    sql: """
                        CREATE TABLE IF NOT EXISTS history_records (
                            id TEXT PRIMARY KEY NOT NULL, wallet_id TEXT, chain_name TEXT NOT NULL, transaction_hash TEXT, created_at REAL NOT NULL, payload BLOB NOT NULL
                        )
                        """, on: configuredHandle
                )
                try Self.execute(
                    sql: "CREATE INDEX IF NOT EXISTS idx_history_records_created_at ON history_records(created_at DESC)", on: configuredHandle
                )
                try Self.execute(
                    sql: "CREATE INDEX IF NOT EXISTS idx_history_records_wallet_id ON history_records(wallet_id)", on: configuredHandle
                )
                try Self.execute(
                    sql: "CREATE INDEX IF NOT EXISTS idx_history_records_chain_name ON history_records(chain_name)", on: configuredHandle
                )
                try Self.execute(
                    sql: "CREATE INDEX IF NOT EXISTS idx_history_records_transaction_hash ON history_records(transaction_hash)", on: configuredHandle
                )
            }
        } catch {
            if let configuredHandle { sqlite3_close(configuredHandle) }
    ...
    func hardResetStorage() { try? clearAll() }
        ...
        return try queue.sync {
            ...
            while true {
                ...
                if result == SQLITE_ROW {
                    ...
                    guard byteCount > 0, let bytes = sqlite3_column_blob(statement, 0) else { continue }
                    let payload = Data(bytes: bytes, count: Int(byteCount))
                    if let decoded = try? decoder.decode(PersistedTransactionRecord.self, from: payload) { records.append(decoded) }
                    continue
                }
    ...
    private func upsert(records: [PersistedTransactionRecord], on databaseHandle: OpaquePointer) throws {
        let sql = """
            INSERT INTO history_records (id, wallet_id, chain_name, transaction_hash, created_at, payload) VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(id) DO UPDATE SET
                wallet_id = excluded.wallet_id, chain_name = excluded.chain_name, transaction_hash = excluded.transaction_hash, created_at = excluded.created_at, payload = excluded.payload
            """
        var statement: OpaquePointer? defer { sqlite3_finalize(statement) }
        try Self.prepare(sql: sql, on: databaseHandle, statement: &statement)
    ...
    private static func beginTransaction(on databaseHandle: OpaquePointer) throws { try execute(sql: "BEGIN IMMEDIATE TRANSACTION", on: databaseHandle) }
    private static func commitTransaction(on databaseHandle: OpaquePointer) throws { try execute(sql: "COMMIT TRANSACTION", on: databaseHandle) }
    private static func rollbackTransaction(on databaseHandle: OpaquePointer) throws { try execute(sql: "ROLLBACK TRANSACTION", on: databaseHandle) }
    private static func execute(sql: String, on databaseHandle: OpaquePointer) throws {
        if sqlite3_exec(databaseHandle, sql, nil, nil, nil) != SQLITE_OK { throw HistoryDatabaseStoreError.sqlite(message: sqliteMessage(from: databaseHandle)) }}
    private static func prepare(sql: String, on databaseHandle: OpaquePointer, statement: inout OpaquePointer?) throws {
        if sqlite3_prepare_v2(databaseHandle, sql, -1, &statement, nil) != SQLITE_OK { throw HistoryDatabaseStoreError.sqlite(message: sqliteMessage(from: databaseHandle)) }}
    private static func reset(_ statement: OpaquePointer?) throws {
        ...
        if sqlite3_reset(statement) != SQLITE_OK { throw HistoryDatabaseStoreError.sqlite(message: "Failed to reset SQLite statement.") }
        if sqlite3_clear_bindings(statement) != SQLITE_OK { throw HistoryDatabaseStoreError.sqlite(message: "Failed to clear SQLite statement bindings.") }}
    private static func step(_ statement: OpaquePointer?, on databaseHandle: OpaquePointer) throws {
        let result = sqlite3_step(statement)
        guard result == SQLITE_DONE else { throw HistoryDatabaseStoreError.sqlite(message: sqliteMessage(from: databaseHandle)) }}
    private static func bind(text: String, at index: Int32, in statement: OpaquePointer?) throws {
        guard sqlite3_bind_text(statement, index, text, -1, SQLITE_TRANSIENT) == SQLITE_OK else { throw HistoryDatabaseStoreError.sqlite(message: "Failed to bind text value.") }}
    private static func bind(optionalText: String?, at index: Int32, in statement: OpaquePointer?) throws {
        guard let optionalText else {
            guard sqlite3_bind_null(statement, index) == SQLITE_OK else { throw HistoryDatabaseStoreError.sqlite(message: "Failed to bind null value.") }
            return
        }
    ...
    private static func bind(double: Double, at index: Int32, in statement: OpaquePointer?) throws {
        guard sqlite3_bind_double(statement, index, double) == SQLITE_OK else { throw HistoryDatabaseStoreError.sqlite(message: "Failed to bind numeric value.") }}
    private static func bind(blob: Data, at index: Int32, in statement: OpaquePointer?) throws {
        let result = blob.withUnsafeBytes { bytes in sqlite3_bind_blob(statement, index, bytes.baseAddress, Int32(blob.count), SQLITE_TRANSIENT) }
        guard result == SQLITE_OK else { throw HistoryDatabaseStoreError.sqlite(message: "Failed to bind blob value.") }}
    private static func sqliteMessage(from databaseHandle: OpaquePointer?) -> String {
        guard let databaseHandle, let cString = sqlite3_errmsg(databaseHandle) else { return "Unknown SQLite error" }
