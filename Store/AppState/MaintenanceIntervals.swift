...
extension WalletStore {
    ...
    func backgroundMaintenanceInterval(now _: Date = Date()) -> TimeInterval {
        ...
        if isConstrainedNetwork || isExpensiveNetwork { interval = max(interval, Self.constrainedBackgroundMaintenanceInterval) }
        if ProcessInfo.processInfo.isLowPowerModeEnabled { interval = max(interval, Self.lowPowerBackgroundMaintenanceInterval) }
        if currentBatteryLevel() < 0.20 { interval = max(interval, Self.lowBatteryBackgroundMaintenanceInterval) }
        return interval
    }
    func canRunHeavyBackgroundRefresh() -> Bool {
        ...
        if ProcessInfo.processInfo.isLowPowerModeEnabled, currentBatteryLevel() < 0.15 { return false }
