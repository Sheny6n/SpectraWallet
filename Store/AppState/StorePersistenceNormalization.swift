...
extension WalletStore {
    ...
    func appendTransaction(_ transaction: TransactionRecord) { transactions.insert(transaction, at: 0) }
    ...
    func upsertStandardUTXOTransactions(_ newTransactions: [TransactionRecord], chainName: String) {
        ...
        for incoming in newTransactions {
            guard incoming.chainName == chainName, let transactionHash = incoming.transactionHash else { continue }
            if let existingIndex = mergedTransactions.firstIndex(where: { existing in
                ...
                mergedTransactions[existingIndex] = TransactionRecord(
                    id: existing.id, walletID: incoming.walletID ?? existing.walletID, kind: incoming.kind, status: incoming.status, walletName: incoming.walletName, assetName: incoming.assetName, symbol: incoming.symbol, chainName: incoming.chainName, amount: incoming.amount, address: incoming.address, transactionHash: incoming.transactionHash, ethereumNonce: incoming.ethereumNonce ?? existing.ethereumNonce, receiptBlockNumber: incoming.receiptBlockNumber ?? existing.receiptBlockNumber, receiptGasUsed: existing.receiptGasUsed, receiptEffectiveGasPriceGwei: existing.receiptEffectiveGasPriceGwei, receiptNetworkFeeETH: existing.receiptNetworkFeeETH, feePriorityRaw: incoming.feePriorityRaw ?? existing.feePriorityRaw, feeRateDescription: incoming.feeRateDescription ?? existing.feeRateDescription, confirmationCount: incoming.confirmationCount ?? existing.confirmationCount, dogecoinConfirmedNetworkFeeDOGE: existing.dogecoinConfirmedNetworkFeeDOGE, dogecoinConfirmations: existing.dogecoinConfirmations, dogecoinFeePriorityRaw: existing.dogecoinFeePriorityRaw, dogecoinEstimatedFeeRateDOGEPerKB: existing.dogecoinEstimatedFeeRateDOGEPerKB, usedChangeOutput: incoming.usedChangeOutput ?? existing.usedChangeOutput, dogecoinUsedChangeOutput: existing.dogecoinUsedChangeOutput, sourceDerivationPath: existing.sourceDerivationPath, changeDerivationPath: existing.changeDerivationPath, sourceAddress: incoming.sourceAddress ?? existing.sourceAddress, changeAddress: incoming.changeAddress ?? existing.changeAddress, dogecoinRawTransactionHex: existing.dogecoinRawTransactionHex, signedTransactionPayload: incoming.signedTransactionPayload ?? existing.signedTransactionPayload, signedTransactionPayloadFormat: incoming.signedTransactionPayloadFormat ?? existing.signedTransactionPayloadFormat, failureReason: existing.failureReason, transactionHistorySource: incoming.transactionHistorySource ?? existing.transactionHistorySource, createdAt: incoming.createdAt
                )
            } else { mergedTransactions.append(incoming) }}
    ...
    func upsertDogecoinTransactions(_ newTransactions: [TransactionRecord]) {
        ...
        var mergedTransactions = transactions
        for incoming in newTransactions {
            guard incoming.chainName == "Dogecoin", let incomingWalletID = incoming.walletID, let transactionHash = incoming.transactionHash else { continue }
            if let existingIndex = mergedTransactions.firstIndex(where: { existing in
                ...
                let existing = mergedTransactions[existingIndex]
                mergedTransactions[existingIndex] = TransactionRecord(
                    id: existing.id, walletID: incoming.walletID ?? existing.walletID, kind: incoming.kind, status: incoming.status, walletName: incoming.walletName, assetName: incoming.assetName, symbol: incoming.symbol, chainName: incoming.chainName, amount: incoming.amount, address: incoming.address, transactionHash: incoming.transactionHash, ethereumNonce: incoming.ethereumNonce ?? existing.ethereumNonce, receiptBlockNumber: incoming.receiptBlockNumber ?? existing.receiptBlockNumber, receiptGasUsed: existing.receiptGasUsed, receiptEffectiveGasPriceGwei: existing.receiptEffectiveGasPriceGwei, receiptNetworkFeeETH: existing.receiptNetworkFeeETH, feePriorityRaw: incoming.feePriorityRaw ?? existing.feePriorityRaw, feeRateDescription: incoming.feeRateDescription ?? existing.feeRateDescription, confirmationCount: incoming.confirmationCount ?? existing.confirmationCount, dogecoinConfirmedNetworkFeeDOGE: incoming.dogecoinConfirmedNetworkFeeDOGE ?? existing.dogecoinConfirmedNetworkFeeDOGE, dogecoinConfirmations: incoming.dogecoinConfirmations ?? existing.dogecoinConfirmations, dogecoinFeePriorityRaw: incoming.dogecoinFeePriorityRaw ?? existing.dogecoinFeePriorityRaw, dogecoinEstimatedFeeRateDOGEPerKB: incoming.dogecoinEstimatedFeeRateDOGEPerKB ?? existing.dogecoinEstimatedFeeRateDOGEPerKB, usedChangeOutput: incoming.usedChangeOutput ?? existing.usedChangeOutput, dogecoinUsedChangeOutput: incoming.dogecoinUsedChangeOutput ?? existing.dogecoinUsedChangeOutput, sourceDerivationPath: incoming.sourceDerivationPath ?? existing.sourceDerivationPath, changeDerivationPath: incoming.changeDerivationPath ?? existing.changeDerivationPath, sourceAddress: incoming.sourceAddress ?? existing.sourceAddress, changeAddress: incoming.changeAddress ?? existing.changeAddress, dogecoinRawTransactionHex: incoming.dogecoinRawTransactionHex ?? existing.dogecoinRawTransactionHex, signedTransactionPayload: incoming.signedTransactionPayload ?? existing.signedTransactionPayload, signedTransactionPayloadFormat: incoming.signedTransactionPayloadFormat ?? existing.signedTransactionPayloadFormat, failureReason: incoming.failureReason ?? existing.failureReason, transactionHistorySource: incoming.transactionHistorySource ?? existing.transactionHistorySource, createdAt: incoming.createdAt
                )
            } else { mergedTransactions.append(incoming) }}
    ...
    func upsertAccountBasedTransactions(_ newTransactions: [TransactionRecord], chainName: String, includeSymbolInIdentity: Bool = false) {
        ...
        var mergedTransactions = transactions
        for incoming in newTransactions {
            guard incoming.chainName == chainName, let incomingWalletID = incoming.walletID, let transactionHash = incoming.transactionHash else { continue }
            if let existingIndex = mergedTransactions.firstIndex(where: { existing in
                existing.chainName == chainName
                    && existing.transactionHash == transactionHash
                    && existing.kind == incoming.kind
                ...
                let existing = mergedTransactions[existingIndex]
                mergedTransactions[existingIndex] = TransactionRecord(
                    id: existing.id, walletID: incoming.walletID ?? existing.walletID, kind: incoming.kind, status: incoming.status, walletName: incoming.walletName, assetName: incoming.assetName, symbol: incoming.symbol, chainName: incoming.chainName, amount: incoming.amount, address: incoming.address, transactionHash: incoming.transactionHash, ethereumNonce: existing.ethereumNonce, receiptBlockNumber: incoming.receiptBlockNumber ?? existing.receiptBlockNumber, receiptGasUsed: existing.receiptGasUsed, receiptEffectiveGasPriceGwei: existing.receiptEffectiveGasPriceGwei, receiptNetworkFeeETH: existing.receiptNetworkFeeETH, feePriorityRaw: incoming.feePriorityRaw ?? existing.feePriorityRaw, feeRateDescription: incoming.feeRateDescription ?? existing.feeRateDescription, confirmationCount: incoming.confirmationCount ?? existing.confirmationCount, dogecoinConfirmedNetworkFeeDOGE: existing.dogecoinConfirmedNetworkFeeDOGE, dogecoinConfirmations: existing.dogecoinConfirmations, dogecoinFeePriorityRaw: existing.dogecoinFeePriorityRaw, dogecoinEstimatedFeeRateDOGEPerKB: existing.dogecoinEstimatedFeeRateDOGEPerKB, usedChangeOutput: incoming.usedChangeOutput ?? existing.usedChangeOutput, dogecoinUsedChangeOutput: existing.dogecoinUsedChangeOutput, sourceDerivationPath: existing.sourceDerivationPath, changeDerivationPath: existing.changeDerivationPath, sourceAddress: incoming.sourceAddress ?? existing.sourceAddress, changeAddress: incoming.changeAddress ?? existing.changeAddress, dogecoinRawTransactionHex: existing.dogecoinRawTransactionHex, signedTransactionPayload: incoming.signedTransactionPayload ?? existing.signedTransactionPayload, signedTransactionPayloadFormat: incoming.signedTransactionPayloadFormat ?? existing.signedTransactionPayloadFormat, failureReason: incoming.failureReason ?? existing.failureReason, transactionHistorySource: incoming.transactionHistorySource ?? existing.transactionHistorySource, createdAt: incoming.createdAt == Date.distantPast ? existing.createdAt : incoming.createdAt
                )
            } else { mergedTransactions.append(incoming) }}
        mergedTransactions.sort { lhs, rhs in
            lhs.createdAt > rhs.createdAt
    ...
    func upsertEVMTransactions(_ newTransactions: [TransactionRecord], chainName: String) {
        ...
        for incoming in newTransactions {
            ...
            if let existingIndex = mergedTransactions.firstIndex(where: { existing in
                ...
                let existing = mergedTransactions[existingIndex]
                mergedTransactions[existingIndex] = TransactionRecord(
                    id: existing.id, walletID: incoming.walletID ?? existing.walletID, kind: incoming.kind, status: incoming.status, walletName: incoming.walletName, assetName: incoming.assetName, symbol: incoming.symbol, chainName: incoming.chainName, amount: incoming.amount, address: incoming.address, transactionHash: incoming.transactionHash, ethereumNonce: incoming.ethereumNonce ?? existing.ethereumNonce, receiptBlockNumber: incoming.receiptBlockNumber ?? existing.receiptBlockNumber, receiptGasUsed: incoming.receiptGasUsed ?? existing.receiptGasUsed, receiptEffectiveGasPriceGwei: incoming.receiptEffectiveGasPriceGwei ?? existing.receiptEffectiveGasPriceGwei, receiptNetworkFeeETH: incoming.receiptNetworkFeeETH ?? existing.receiptNetworkFeeETH, feePriorityRaw: incoming.feePriorityRaw ?? existing.feePriorityRaw, feeRateDescription: incoming.feeRateDescription ?? existing.feeRateDescription, confirmationCount: incoming.confirmationCount ?? existing.confirmationCount, dogecoinConfirmedNetworkFeeDOGE: existing.dogecoinConfirmedNetworkFeeDOGE, dogecoinConfirmations: existing.dogecoinConfirmations, dogecoinFeePriorityRaw: existing.dogecoinFeePriorityRaw, dogecoinEstimatedFeeRateDOGEPerKB: existing.dogecoinEstimatedFeeRateDOGEPerKB, usedChangeOutput: incoming.usedChangeOutput ?? existing.usedChangeOutput, dogecoinUsedChangeOutput: existing.dogecoinUsedChangeOutput, sourceDerivationPath: existing.sourceDerivationPath, changeDerivationPath: existing.changeDerivationPath, sourceAddress: existing.sourceAddress, changeAddress: existing.changeAddress, dogecoinRawTransactionHex: existing.dogecoinRawTransactionHex, signedTransactionPayload: incoming.signedTransactionPayload ?? existing.signedTransactionPayload, signedTransactionPayloadFormat: incoming.signedTransactionPayloadFormat ?? existing.signedTransactionPayloadFormat, failureReason: incoming.failureReason ?? existing.failureReason, transactionHistorySource: incoming.transactionHistorySource ?? existing.transactionHistorySource, createdAt: incoming.createdAt == Date.distantPast ? existing.createdAt : incoming.createdAt
                )
            } else { mergedTransactions.append(incoming) }}
        mergedTransactions.sort { lhs, rhs in
            lhs.createdAt > rhs.createdAt
    ...
    func clearDeletedWalletDiagnostics(walletID: UUID, chainName: String, hasRemainingWalletsOnChain: Bool) {
    ...
    func persistChainOwnedAddressMap() {
        ...
        for (chainName, addressMap) in chainOwnedAddressMapByChain {
            for (_, record) in addressMap {
                persistOwnedAddressToRust(
                    walletId: record.walletID.uuidString, chainName: chainName, address: record.address ?? "", derivationPath: record.derivationPath, branch: record.branch, branchIndex: record.index
                )
            }}}
...
private extension WalletStore {
        ...
        let request = WalletRustStoreDerivedStateRequest(
            wallets: wallets.map { wallet in
                ...
                return WalletRustStoreDerivedWalletInput(
                    walletID: wallet.id.uuidString, includeInPortfolioTotal: wallet.includeInPortfolioTotal, hasSigningMaterial: signingMaterial.hasSigningMaterial, isPrivateKeyBacked: signingMaterial.isPrivateKeyBacked, holdings: wallet.holdings.enumerated().map { index, holding in
    ...
    func resolveHolding(_ reference: WalletRustWalletHoldingRef, in wallets: [ImportedWallet]) -> Coin? {
        guard let wallet = cachedWalletByIDString[reference.walletID]
            ?? wallets.first(where: { $0.id.uuidString == reference.walletID }), wallet.holdings.indices.contains(reference.holdingIndex) else {
            return nil
        }
        ...
        let request = WalletRustTransferAvailabilityRequest(
            wallets: wallets.map { wallet in
                ...
                return WalletRustTransferWalletInput(
                    walletID: wallet.id.uuidString, hasSigningMaterial: hasSigningMaterial, holdings: wallet.holdings.enumerated().map { index, holding in
