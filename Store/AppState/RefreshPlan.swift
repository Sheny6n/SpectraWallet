...
struct WalletRefreshPlanner {
    static func activeMaintenancePlan(now: Date, lastPendingTransactionRefreshAt: Date?, lastLivePriceRefreshAt: Date?, hasPendingTransactionMaintenanceWork: Bool, shouldRunScheduledPriceRefresh: Bool, pendingRefreshInterval: TimeInterval, priceRefreshInterval: TimeInterval) -> WalletActiveMaintenancePlan {
        do {
            ...
            let refreshPendingTransactions: Bool
            if let lastPendingTransactionRefreshAt { refreshPendingTransactions = hasPendingTransactionMaintenanceWork && now.timeIntervalSince(lastPendingTransactionRefreshAt) >= pendingRefreshInterval } else { refreshPendingTransactions = hasPendingTransactionMaintenanceWork }
            let refreshLivePrices: Bool
            if let lastLivePriceRefreshAt { refreshLivePrices = shouldRunScheduledPriceRefresh && now.timeIntervalSince(lastLivePriceRefreshAt) >= priceRefreshInterval } else { refreshLivePrices = shouldRunScheduledPriceRefresh }
            return WalletActiveMaintenancePlan(
                refreshPendingTransactions: refreshPendingTransactions, refreshLivePrices: refreshLivePrices
            )
        }}
        ...
        do {
            ...
            return chainIDs.sorted().compactMap { chainID in
                    ...
                    let refreshHistory = includeHistoryRefreshes && shouldRefreshOnChainHistory(
                        for: chainID, now: now, interval: historyRefreshInterval, lastHistoryRefreshAtByChainID: lastHistoryRefreshAtByChainID
                    )
    ...
    private static func shouldRefreshChainData(_ chainID: WalletChainID, now: Date, force: Bool, pendingTransactionMaintenanceChains: Set<WalletChainID>, degradedChains: Set<WalletChainID>, lastGoodChainSyncByID: [WalletChainID: Date], automaticChainRefreshStalenessInterval: TimeInterval) -> Bool {
        if force { return true }
        if pendingTransactionMaintenanceChains.contains(chainID) { return true }
        if degradedChains.contains(chainID) { return true }
        guard let lastGoodSyncAt = lastGoodChainSyncByID[chainID] else { return true }
        return now.timeIntervalSince(lastGoodSyncAt) >= automaticChainRefreshStalenessInterval
    }
    private static func shouldRefreshOnChainHistory(for chainID: WalletChainID, now: Date, interval: TimeInterval, lastHistoryRefreshAtByChainID: [WalletChainID: Date]) -> Bool {
        guard let lastRefreshAt = lastHistoryRefreshAtByChainID[chainID] else { return true }
