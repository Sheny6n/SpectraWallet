...
extension WalletStore {
    ...
    func reloadPersistedStateFromSQLite() async {
        ...
        if let alertsPayload = await loadCodableFromSQLite(PersistedPriceAlertStore.self, key: Self.priceAlertsDefaultsKey), alertsPayload.version == PersistedPriceAlertStore.currentVersion { priceAlerts = alertsPayload.alerts.map(PriceAlertRule.init(snapshot:)) }
        if let abPayload = await loadCodableFromSQLite(PersistedAddressBookStore.self, key: Self.addressBookDefaultsKey), abPayload.version == PersistedAddressBookStore.currentVersion {
            addressBook = abPayload.entries.map { AddressBookEntry(id: $0.id, name: $0.name, chainName: $0.chainName, address: $0.address, note: $0.note) }}
    ...
    func persistAddressBook() {
        let payload = PersistedAddressBookStore(
            version: PersistedAddressBookStore.currentVersion, entries: addressBook.map {
                PersistedAddressBookEntry(
                    id: $0.id, name: $0.name, chainName: $0.chainName, address: $0.address, note: $0.note
                )
            }
        )
    ...
    func loadPersistedAddressBook() -> [AddressBookEntry] {
        guard let payload = loadCodableFromUserDefaults(PersistedAddressBookStore.self, key: Self.addressBookDefaultsKey) else { return [] }
        guard payload.version == PersistedAddressBookStore.currentVersion else { return [] }
    ...
    func loadPersistedPriceAlerts() -> [PriceAlertRule] {
        guard let payload = loadCodableFromUserDefaults(PersistedPriceAlertStore.self, key: Self.priceAlertsDefaultsKey), payload.version == PersistedPriceAlertStore.currentVersion else { return [] }
