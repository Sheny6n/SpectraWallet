    ...
    private func dashboardPinOptionUncached(
        ...
        if let representativeCoin = portfolioCoins.first {
            ...
            return DashboardPinOption(
                symbol: normalizedSymbol, name: representativeCoin.name, subtitle: chainNames.isEmpty ? representativeCoin.chainName : chainNames.joined(separator: ", "), assetIdentifier: representativeCoin.iconIdentifier, mark: representativeCoin.mark, color: representativeCoin.color
            )
        }
        if let representativeEntry = trackedEntries.first {
            ...
            return DashboardPinOption(
                ...
                ), mark: Coin.displayMark(for: representativeEntry.symbol), color: Coin.displayColor(for: representativeEntry.symbol)
            )
        }
        if let prototype {
            return DashboardPinOption(
                symbol: normalizedSymbol, name: prototype.name, subtitle: prototype.chainName, assetIdentifier: prototype.iconIdentifier, mark: prototype.mark, color: prototype.color
            )
        }
        return nil
    }
