...
extension WalletStore {
    ...
    private func supportedDecimalPlaces(for symbol: String, chainName: String) -> Int {
        if let trackedToken = cachedTokenPreferenceByChainAndSymbol[tokenPreferenceLookupKey(chainName: chainName, symbol: symbol)] { return trackedToken.decimals }
        switch chainName {
        case "Bitcoin", "Bitcoin Cash", "Bitcoin SV", "Litecoin", "Dogecoin", "Aptos": return 8
        case "Ethereum", "Ethereum Classic", "Arbitrum", "Optimism", "BNB Chain", "Avalanche", "Hyperliquid": return 18
        case "Tron", "Cardano", "XRP Ledger": return 6
        case "Solana", "Sui", "TON": return 9
        case "Monero": return 12
        case "NEAR": return 24
    ...
    func defaultAssetDisplayDecimalsByChain(defaultValue: Int = 3) -> [String: Int] {
        ...
        let chains = ["Bitcoin", "Bitcoin Cash", "Bitcoin SV", "Litecoin", "Dogecoin", "Ethereum", "Ethereum Classic", "Arbitrum", "Optimism", "BNB Chain", "Avalanche", "Hyperliquid", "Tron", "Solana", "Cardano", "XRP Ledger", "Monero", "Sui", "Aptos", "TON", "NEAR", "Polkadot"]
        return Dictionary(uniqueKeysWithValues: chains.map { ($0, normalized) })
    }
    private func nativeAssetDisplaySettingsKey(for chainName: String) -> String { ["Ethereum", "Arbitrum", "Optimism"].contains(chainName) ? "Ethereum" : chainName }
    private func rebuildNormalizedHistoryIndexUsingRust(walletByID: [UUID: ImportedWallet]) -> [NormalizedHistoryEntry] {
        do {
            let request = WalletRustNormalizeHistoryRequest(
                wallets: walletByID.map {
                    WalletRustHistoryWallet(
                        walletID: $0.key.uuidString.lowercased(), selectedChain: $0.value.selectedChain
                    )
                }, transactions: transactions.map {
                    WalletRustHistoryTransaction(
                        id: $0.id.uuidString.lowercased(), walletID: $0.walletID?.uuidString.lowercased(), kind: $0.kind.rawValue, status: $0.status.rawValue, walletName: $0.walletName, assetName: $0.assetName, symbol: $0.symbol, chainName: $0.chainName, address: $0.address, transactionHash: $0.transactionHash, transactionHistorySource: $0.transactionHistorySource, createdAtUnix: $0.createdAt.timeIntervalSince1970
                    )
            ...
            let deduped = groupedByDedupeKey.values.compactMap { entries -> NormalizedHistoryEntry? in
                ...
                return NormalizedHistoryEntry(
                    id: best.id, transactionID: best.transactionID, dedupeKey: best.dedupeKey, createdAt: best.createdAt, kind: best.kind, status: best.status, walletName: best.walletName, assetName: best.assetName, symbol: best.symbol, chainName: best.chainName, address: best.address, transactionHash: best.transactionHash, sourceTag: best.sourceTag, providerCount: providerCount, searchIndex: best.searchIndex
                )
            }
    ...
    private func normalizedHistoryEntryFallback(for transaction: TransactionRecord) -> NormalizedHistoryEntry {
        ...
        let searchIndex = [
            transaction.walletName, transaction.assetName, transaction.symbol, transaction.chainName, transaction.address, transaction.transactionHash ?? "", sourceTag
        ].joined(separator: " ").lowercased()
        return NormalizedHistoryEntry(
            id: stableID, transactionID: transaction.id, dedupeKey: dedupeKey, createdAt: transaction.createdAt, kind: transaction.kind, status: transaction.status, walletName: transaction.walletName, assetName: transaction.assetName, symbol: transaction.symbol, chainName: transaction.chainName, address: transaction.address, transactionHash: transaction.transactionHash, sourceTag: sourceTag, providerCount: 1, searchIndex: searchIndex
        )
    }
