...
func decodeBitcoinHistoryPageFromRust(json: String, limit: Int) -> BitcoinHistoryPage {
    guard let data = json.data(using: .utf8), let arr = try? JSONSerialization.jsonObject(with: data) as? [[String: Any]] else { return BitcoinHistoryPage(snapshots: [], nextCursor: nil, sourceUsed: "rust") }
    let snapshots: [BitcoinHistorySnapshot] = arr.prefix(limit).compactMap { obj in
        ...
        return BitcoinHistorySnapshot(
            txid: txid, amountBTC: amountBTC, kind: kind, status: status, counterpartyAddress: "", blockHeight: blockHeight, createdAt: createdAt
        )
    }
...
func solanaSymbolAndAssetName(
    mint: String, rawSymbol: String, tokenMeta: [String: SolanaBalanceService.KnownTokenMetadata]
) -> (String, String) {
    if mint.isEmpty { return ("SOL", "Solana") }
    if let meta = tokenMeta[mint] { return (meta.symbol, meta.name) }
...
private struct SimpleChainHistoryDescriptor {
    ...
    let resolveAddress: (WalletStore, ImportedWallet) -> String? let parseEntry: ([String: Any]) -> (amount: Double, counterparty: String, txhash: String, blockHeight: Int?, timestamp: Double, isIncoming: Bool, isConfirmed: Bool)? let upsert: (WalletStore, [TransactionRecord]) -> Void
}
private let _simpleChainDescriptors: [SimpleChainHistoryDescriptor] = {
    let isoParser = ISO8601DateFormatter()
    return [
        SimpleChainHistoryDescriptor(
            chainName: "Bitcoin Cash", chainId: SpectraChainID.bitcoinCash, historyChainId: HistoryChainID.bitcoinCash, assetName: "Bitcoin Cash", symbol: "BCH", historySource: "rust", resolveAddress: { s, w in s.resolvedBitcoinCashAddress(for: w) }, parseEntry: { e in
                guard let txid = e["txid"] as? String else { return nil }
                let sat = (e["amount_sat"] as? Int) ?? 0
                let bh = e["block_height"] as? Int
                return (Double(abs(sat)) / 1e8, "", txid, bh, (e["timestamp"] as? Double) ?? 0, (e["is_incoming"] as? Bool) ?? (sat >= 0), (bh ?? 0) > 0)
            }, upsert: { s, r in s.upsertBitcoinCashTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "Bitcoin SV", chainId: SpectraChainID.bitcoinSv, historyChainId: HistoryChainID.bitcoinSV, assetName: "Bitcoin SV", symbol: "BSV", historySource: "rust", resolveAddress: { s, w in s.resolvedBitcoinSVAddress(for: w) }, parseEntry: { e in
                guard let txid = e["txid"] as? String else { return nil }
                let sat = (e["amount_sat"] as? Int) ?? 0
                let bh = e["block_height"] as? Int
                return (Double(abs(sat)) / 1e8, "", txid, bh, (e["timestamp"] as? Double) ?? 0, (e["is_incoming"] as? Bool) ?? (sat >= 0), (bh ?? 0) > 0)
            ...
            chainName: "Litecoin", chainId: SpectraChainID.litecoin, historyChainId: HistoryChainID.litecoin, assetName: "Litecoin", symbol: "LTC", historySource: "rust", resolveAddress: { s, w in s.resolvedLitecoinAddress(for: w) }, parseEntry: { e in
                guard let txid = e["txid"] as? String else { return nil }
                let sat = (e["amount_sat"] as? Int) ?? 0
                let bh = e["block_height"] as? Int
                return (Double(abs(sat)) / 1e8, "", txid, bh, (e["timestamp"] as? Double) ?? 0, (e["is_incoming"] as? Bool) ?? (sat >= 0), (bh ?? 0) > 0)
            ...
            chainName: "Cardano", chainId: SpectraChainID.cardano, historyChainId: UInt32.max, assetName: "Cardano", symbol: "ADA", historySource: "blockfrost", resolveAddress: { s, w in s.resolvedCardanoAddress(for: w) }, parseEntry: { e in
                let lovelace = (e["amount_lovelace"] as? Int) ?? 0
                return (Double(abs(lovelace)) / 1_000_000, "", e["txid"] as? String ?? "", nil, (e["block_time"] as? Double) ?? 0, (e["is_incoming"] as? Bool) ?? false, true)
            }, upsert: { s, r in s.upsertCardanoTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "XRP Ledger", chainId: SpectraChainID.xrp, historyChainId: UInt32.max, assetName: "XRP", symbol: "XRP", historySource: "xrpscan", resolveAddress: { s, w in s.resolvedXRPAddress(for: w) }, parseEntry: { e in
                let isIncoming = (e["is_incoming"] as? Bool) ?? false
                let drops = (e["amount_drops"] as? Int) ?? 0
                return (Double(drops) / 1_000_000, isIncoming ? (e["from"] as? String ?? "") : (e["to"] as? String ?? ""), e["txid"] as? String ?? "", nil, (e["timestamp"] as? Double) ?? 0, isIncoming, true)
            }, upsert: { s, r in s.upsertXRPTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "Stellar", chainId: SpectraChainID.stellar, historyChainId: UInt32.max, assetName: "Stellar Lumens", symbol: "XLM", historySource: "horizon", resolveAddress: { s, w in s.resolvedStellarAddress(for: w) }, parseEntry: { [isoParser] e in
                let isIncoming = (e["is_incoming"] as? Bool) ?? false
                let stroops = (e["amount_stroops"] as? Int) ?? 0
                let ts: Double
                if let str = e["timestamp"] as? String { ts = isoParser.date(from: str)?.timeIntervalSince1970 ?? 0 } else { ts = (e["timestamp"] as? Double) ?? 0 }
                return (Double(abs(stroops)) / 10_000_000, isIncoming ? (e["from"] as? String ?? "") : (e["to"] as? String ?? ""), e["txid"] as? String ?? "", nil, ts, isIncoming, true)
            }, upsert: { s, r in s.upsertStellarTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "Monero", chainId: SpectraChainID.monero, historyChainId: UInt32.max, assetName: "Monero", symbol: "XMR", historySource: "monero-rpc", resolveAddress: { s, w in s.resolvedMoneroAddress(for: w) }, parseEntry: { e in
                let piconeros = (e["amount_piconeros"] as? Double) ?? 0
                return (piconeros / 1_000_000_000_000, "", e["txid"] as? String ?? "", nil, (e["timestamp"] as? Double) ?? 0, (e["is_incoming"] as? Bool) ?? false, true)
            }, upsert: { s, r in s.upsertMoneroTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "Sui", chainId: SpectraChainID.sui, historyChainId: UInt32.max, assetName: "Sui", symbol: "SUI", historySource: "sui-rpc", resolveAddress: { s, w in s.resolvedSuiAddress(for: w) }, parseEntry: { e in
                let mist = (e["amount_mist"] as? Double) ?? 0
                let tsMs = (e["timestamp_ms"] as? Double) ?? 0
                return (mist / 1_000_000_000, "", e["digest"] as? String ?? "", nil, tsMs / 1000, (e["is_incoming"] as? Bool) ?? false, true)
            }, upsert: { s, r in s.upsertSuiTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "Internet Computer", chainId: SpectraChainID.icp, historyChainId: UInt32.max, assetName: "Internet Computer", symbol: "ICP", historySource: "rosetta", resolveAddress: { s, w in s.resolvedICPAddress(for: w) }, parseEntry: { e in
                let isIncoming = (e["is_incoming"] as? Bool) ?? false
                let e8s = (e["amount_e8s"] as? Double) ?? 0
                let blockIndex = (e["block_index"] as? Int) ?? 0
                let tsNs = (e["timestamp_ns"] as? Double) ?? 0
                return (e8s / 100_000_000, isIncoming ? (e["from"] as? String ?? "") : (e["to"] as? String ?? ""), String(blockIndex), nil, tsNs / 1_000_000_000, isIncoming, true)
            }, upsert: { s, r in s.upsertICPTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "Aptos", chainId: SpectraChainID.aptos, historyChainId: UInt32.max, assetName: "Aptos", symbol: "APT", historySource: "aptos-rpc", resolveAddress: { s, w in s.resolvedAptosAddress(for: w) }, parseEntry: { e in
                let isIncoming = (e["is_incoming"] as? Bool) ?? false
                let octas = (e["amount_octas"] as? Double) ?? 0
                let tsUs = (e["timestamp_us"] as? Double) ?? 0
                return (octas / 100_000_000, isIncoming ? (e["from"] as? String ?? "") : (e["to"] as? String ?? ""), e["txid"] as? String ?? "", nil, tsUs / 1_000_000, isIncoming, true)
            }, upsert: { s, r in s.upsertAptosTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "TON", chainId: SpectraChainID.ton, historyChainId: UInt32.max, assetName: "Toncoin", symbol: "TON", historySource: "tonapi", resolveAddress: { s, w in s.resolvedTONAddress(for: w) }, parseEntry: { e in
                let isIncoming = (e["is_incoming"] as? Bool) ?? false
                let nanotons = (e["amount_nanotons"] as? Double) ?? 0
                return (nanotons / 1_000_000_000, isIncoming ? (e["from"] as? String ?? "") : (e["to"] as? String ?? ""), e["txid"] as? String ?? "", nil, (e["timestamp"] as? Double) ?? 0, isIncoming, true)
            }, upsert: { s, r in s.upsertTONTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "NEAR", chainId: SpectraChainID.near, historyChainId: UInt32.max, assetName: "NEAR Protocol", symbol: "NEAR", historySource: "near-rpc", resolveAddress: { s, w in s.resolvedNearAddress(for: w) }, parseEntry: { e in
                let isIncoming = (e["is_incoming"] as? Bool) ?? false
                let yoctoStr = e["amount_yocto"] as? String ?? "0"
                let tsNs = (e["timestamp_ns"] as? Double) ?? 0
                return ((Double(yoctoStr) ?? 0) / 1e24, isIncoming ? (e["signer_id"] as? String ?? "") : (e["receiver_id"] as? String ?? ""), e["txid"] as? String ?? "", nil, tsNs / 1_000_000_000, isIncoming, true)
            }, upsert: { s, r in s.upsertNearTransactions(r) }
        ), SimpleChainHistoryDescriptor(
            chainName: "Polkadot", chainId: SpectraChainID.polkadot, historyChainId: UInt32.max, assetName: "Polkadot", symbol: "DOT", historySource: "subscan", resolveAddress: { s, w in s.resolvedPolkadotAddress(for: w) }, parseEntry: { e in
                let isIncoming = (e["is_incoming"] as? Bool) ?? false
                let planck = (e["amount_planck"] as? Double) ?? 0
                return (planck / 10_000_000_000, isIncoming ? (e["from"] as? String ?? "") : (e["to"] as? String ?? ""), e["txid"] as? String ?? "", nil, (e["timestamp"] as? Double) ?? 0, isIncoming, true)
            }, upsert: { s, r in s.upsertPolkadotTransactions(r) }
        ), ]
}()
extension WalletStore {
private func refreshSimpleChainTransactions(
    descriptor d: SimpleChainHistoryDescriptor, limit: Int? = nil, loadMore: Bool = false, targetWalletIDs: Set<UUID>? = nil
) async {
    let walletSnapshot = wallets
    let filteredWallets = walletSnapshot.filter { wallet in
        guard wallet.selectedChain == d.chainName, d.resolveAddress(self, wallet) != nil else { return false }
        guard let targetWalletIDs else { return true }
        return targetWalletIDs.contains(wallet.id)
    }
    ...
    if !loadMore && usesPagination {
        for walletID in Set(filteredWallets.map(\.id)) { resetHistoryPagination(chainId: d.historyChainId, walletId: walletID) }}
    var discoveredTransactions: [TransactionRecord] = []
    var encounteredErrors = false
    for wallet in filteredWallets {
        if usesPagination && loadMore && historyPaginationExhausted(chainId: d.historyChainId, walletId: wallet.id) { continue }
        ...
        do {
            ...
            let records = decodeRustHistoryJSON(json: json).prefix(requestedLimit).compactMap { entry -> TransactionRecord? in
                ...
                return TransactionRecord(
                    walletID: wallet.id, kind: parsed.isIncoming ? .receive : .send, status: parsed.isConfirmed ? .confirmed : .pending, walletName: wallet.name, assetName: d.assetName, symbol: d.symbol, chainName: d.chainName, amount: parsed.amount, address: parsed.counterparty, transactionHash: parsed.txhash, receiptBlockNumber: parsed.blockHeight, transactionHistorySource: d.historySource, createdAt: parsed.timestamp > 0 ? Date(timeIntervalSince1970: parsed.timestamp) : Date()
                )
            }
    ...
    if !discoveredTransactions.isEmpty {
        ...
        if encounteredErrors { markChainDegraded(d.chainName, detail: "\(d.chainName) history loaded with partial provider failures.") } else { markChainHealthy(d.chainName) }
    } else if encounteredErrors { markChainDegraded(d.chainName, detail: "\(d.chainName) history refresh failed. Using cached history.") }
}
...
func refreshTronTransactions(loadMore: Bool = false, targetWalletIDs: Set<UUID>? = nil) async {
    ...
    let tronWallets = walletSnapshot.filter { wallet in
        guard wallet.selectedChain == "Tron", resolvedTronAddress(for: wallet) != nil else { return false }
        guard let targetWalletIDs else { return true }
        return targetWalletIDs.contains(wallet.id)
    }
    guard !tronWallets.isEmpty else { return }
    if !loadMore {
        for walletID in Set(tronWallets.map(\.id)) { resetHistoryPagination(chainId: HistoryChainID.tron, walletId: walletID) }}
    var discoveredTransactions: [TransactionRecord] = []
    var encounteredErrors = false
    for wallet in tronWallets {
        if loadMore && historyPaginationExhausted(chainId: HistoryChainID.tron, walletId: wallet.id) { continue }
        guard let tronAddress = resolvedTronAddress(for: wallet) else { continue }
        do {
            let json = try await WalletServiceBridge.shared.fetchHistoryJSON(chainId: SpectraChainID.tron, address: tronAddress)
            let entries = decodeRustHistoryJSON(json: json)
            if entries.isEmpty { markHistoryExhausted(chainId: HistoryChainID.tron, walletId: wallet.id) } else { markHistoryActive(chainId: HistoryChainID.tron, walletId: wallet.id) }
            let records = entries.map { entry -> TransactionRecord in
                ...
                return TransactionRecord(
                    walletID: wallet.id, kind: isIncoming ? .receive : .send, status: .confirmed, walletName: wallet.name, assetName: tronAssetName(symbol: symbol), symbol: symbol, chainName: "Tron", amount: amount, address: counterparty, transactionHash: txid, transactionHistorySource: "tronscan", createdAt: timestampMs > 0 ? Date(timeIntervalSince1970: timestampMs / 1000) : Date()
                )
            }
            discoveredTransactions.append(contentsOf: records)
        } catch {
    ...
    if !discoveredTransactions.isEmpty {
        ...
        if encounteredErrors { markChainDegraded("Tron", detail: "Tron history loaded with partial provider failures.") } else { markChainHealthy("Tron") }
    } else if encounteredErrors { markChainDegraded("Tron", detail: "Tron history refresh failed. Using cached history.") }
}
func refreshSolanaTransactions(loadMore: Bool = false) async {
    let walletSnapshot = wallets
    let solanaWallets = walletSnapshot.filter { wallet in wallet.selectedChain == "Solana" && resolvedSolanaAddress(for: wallet) != nil }
    ...
    for wallet in solanaWallets {
        ...
        do {
            let json = try await WalletServiceBridge.shared.fetchHistoryJSON(chainId: SpectraChainID.solana, address: solanaAddress)
            let entries = decodeRustHistoryJSON(json: json)
            ...
            let records = entries.compactMap { entry -> TransactionRecord? in
                ...
                let (resolvedSymbol, assetName) = solanaSymbolAndAssetName(mint: mint, rawSymbol: symbol, tokenMeta: tokenMeta)
                return TransactionRecord(
                    walletID: wallet.id, kind: isIncoming ? .receive : .send, status: .confirmed, walletName: wallet.name, assetName: assetName, symbol: resolvedSymbol, chainName: "Solana", amount: amount, address: counterparty, transactionHash: sig, transactionHistorySource: "solana-rpc", createdAt: timestampSec > 0 ? Date(timeIntervalSince1970: timestampSec) : Date()
                )
            }
            discoveredTransactions.append(contentsOf: records)
        } catch {
    ...
    if !discoveredTransactions.isEmpty {
        ...
        if encounteredErrors { markChainDegraded("Solana", detail: "Solana history loaded with partial provider failures.") } else { markChainHealthy("Solana") }
    } else if encounteredErrors {
        let hasCachedHistory = transactions.contains { transaction in
            guard transaction.chainName == "Solana", let walletID = transaction.walletID else { return false }
            return refreshedWalletIDs.contains(walletID)
        }
        if hasCachedHistory { markChainDegraded("Solana", detail: "Solana history refresh failed. Using cached history.") }}
}
...
func refreshEVMTokenTransactions(
    ...
    for (targetWallets, _, normalizedAddress) in historyTargets {
        ...
        var tokenDiagnostics: EthereumTokenTransferHistoryDiagnostics? var tokenHistoryError: Error? var nativeTransfers: [EthereumNativeTransferSnapshot] = []
        ...
        let diagsKP: WritableKeyPath<WalletStore, DiagsByWallet>? =
            ...
            : nil
        let tsKP: WritableKeyPath<WalletStore, Date?>? =
        ...
        if let diagsKP, let tsKP {
            if let tokenDiagnostics {
                for wallet in targetWallets { self[keyPath: diagsKP][wallet.id] = tokenDiagnostics }
                ...
                for wallet in targetWallets { self[keyPath: diagsKP][wallet.id] = errDiag }}
...
func refreshDogecoinTransactions(limit: Int? = nil, loadMore: Bool = false, targetWalletIDs: Set<UUID>? = nil) async {
    ...
    for (wallet, dogecoinAddresses) in walletsToRefresh {
        ...
        for dogecoinAddress in dogecoinAddresses {
            do {
                let json = try await WalletServiceBridge.shared.fetchHistoryJSON(chainId: SpectraChainID.dogecoin, address: dogecoinAddress)
                let entries = decodeRustHistoryJSON(json: json)
                for entry in entries {
                    ...
                    snapshotsByHash[txid, default: []].append(
                        DogecoinBalanceService.AddressTransactionSnapshot(
                            hash: txid, kind: isIncoming ? .receive : .send, status: (blockHeight ?? 0) > 0 ? .confirmed : .pending, amount: Double(abs(amountKoin)) / 1e8, counterpartyAddress: "", createdAt: timestamp > 0 ? Date(timeIntervalSince1970: timestamp) : Date.distantPast, blockNumber: blockHeight
                        )
                    )
                }
        ...
        let mapped: [TransactionRecord] = snapshotsByHash.values.compactMap { groupedSnapshots -> TransactionRecord? in
            ...
            return TransactionRecord(
                walletID: wallet.id, kind: effectiveKind, status: effectiveStatus, walletName: wallet.name, assetName: "Dogecoin", symbol: "DOGE", chainName: "Dogecoin", amount: effectiveAmount, address: preferredCounterparty, transactionHash: first.hash, receiptBlockNumber: effectiveBlockNumber, receiptGasUsed: nil, receiptEffectiveGasPriceGwei: nil, receiptNetworkFeeETH: nil, failureReason: nil, transactionHistorySource: "dogecoin.providers", createdAt: effectiveCreatedAt
            )
        }
