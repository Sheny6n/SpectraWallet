...
extension WalletStore {
    ...
    private static func decodeEVMReceipt(json: String, txHash: String) throws -> EthereumTransactionReceipt {
        guard let data = json.data(using: .utf8), let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { throw NSError(domain: "EvmReceipt", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid receipt JSON"]) }
        ...
        return EthereumTransactionReceipt(
            transactionHash: txHash, blockNumber: blockNumber, status: status, gasUsed: gasUsed, effectiveGasPriceWei: effectiveGasPriceWei
        )
    }
}
