...
func resetImportForm() {
    ...
    func canRevealSeedPhrase(for walletID: UUID) -> Bool { storedSeedPhrase(for: walletID) != nil }
    ...
    func selectedReceiveCoin(for walletID: String) -> Coin? {
        ...
        if let plan = rustReceiveSelectionPlan(for: walletID, coins: receiveCoins) {
            guard let selectedIndex = plan.selectedReceiveHoldingIndex, receiveCoins.indices.contains(selectedIndex) else { return nil }
