...
class WalletStore: ObservableObject {
    ...
    static func seedPhraseAccount(for walletID: UUID) -> String { "wallet.seed.\(walletID.uuidString)" }
    static func seedPhrasePasswordAccount(for walletID: UUID) -> String { "wallet.seed.password.\(walletID.uuidString)" }
    static func privateKeyAccount(for walletID: UUID) -> String { "wallet.privatekey.\(walletID.uuidString)" }
    func resolvedSeedPhraseAccount(for walletID: UUID) -> String { cachedSecretDescriptorsByWalletID[walletID]?.seedPhraseStoreKey ?? Self.seedPhraseAccount(for: walletID) }
    func resolvedSeedPhrasePasswordAccount(for walletID: UUID) -> String { cachedSecretDescriptorsByWalletID[walletID]?.passwordStoreKey ?? Self.seedPhrasePasswordAccount(for: walletID) }
    func resolvedPrivateKeyAccount(for walletID: UUID) -> String { cachedSecretDescriptorsByWalletID[walletID]?.privateKeyStoreKey ?? Self.privateKeyAccount(for: walletID) }
    ...
    var bitcoinEsploraEndpointsValidationError: String? {
        for endpoint in parsedBitcoinEsploraEndpoints() {
            guard let url = URL(string: endpoint), let scheme = url.scheme?.lowercased(), ["http", "https"].contains(scheme), url.host != nil else { return "Bitcoin Esplora endpoints must be valid http(s) URLs separated by commas." }}
        return nil
    }
    func parseDogecoinAmountInput(_ amountText: String) -> Double? {
        let trimmed = amountText.trimmingCharacters(in: .whitespacesAndNewlines)
        let pattern = #"^(\d+(\.\d{1,8})?|\.\d{1,8})$"#
        guard trimmed.range(of: pattern, options: .regularExpression) != nil, let amount = Double(trimmed), amount > 0 else { return nil }
        return amount
    }
    ...
    func applySendVerificationStatus(_ verificationStatus: SendBroadcastVerificationStatus, chainName: String) {
        ...
        case .verified: clearSendVerificationNotice()
        case .deferred: setDeferredSendVerificationNotice(for: chainName)
        case .failed(let message): setFailedSendVerificationNotice("Broadcast succeeded, but post-broadcast verification reported: \(message)")
        }}
    func updateSendVerificationNoticeForLastSentTransaction() {
        ...
        guard let transactionHash = lastSentTransaction.transactionHash? .trimmingCharacters(in: .whitespacesAndNewlines), !transactionHash.isEmpty else {
            clearSendVerificationNotice()
            return
        }
        ...
        let wasObservedOnNetwork = lastSentTransaction.status == .confirmed || lastSentTransaction.transactionHistorySource != nil || lastSentTransaction.receiptBlockNumber != nil || (lastSentTransaction.dogecoinConfirmations ?? 0) > 0
    ...
    func enabledEthereumTrackedTokens() -> [EthereumSupportedToken] {
        enabledTokenPreferences(for: .ethereum).map { entry in
            EthereumSupportedToken(
                name: entry.name, symbol: entry.symbol, contractAddress: normalizeEVMAddress(entry.contractAddress), decimals: entry.decimals, marketDataID: entry.marketDataID, coinGeckoID: entry.coinGeckoID
            )
        }}
    ...
    func enabledTronTrackedTokens() -> [TronBalanceService.TrackedTRC20Token] {
        enabledTokenPreferences(for: .tron).map { entry in
            TronBalanceService.TrackedTRC20Token(
                symbol: entry.symbol, contractAddress: entry.contractAddress, decimals: entry.decimals
            )
        }}
    func solanaTrackedTokens(includeDisabled: Bool = false) -> [String: SolanaBalanceService.KnownTokenMetadata] {
        ...
        for entry in entries {
            result[entry.contractAddress] = SolanaBalanceService.KnownTokenMetadata(
                symbol: entry.symbol, name: entry.name, decimals: entry.decimals, marketDataID: entry.marketDataID, coinGeckoID: entry.coinGeckoID
            )
        }
        return result
    }
            ...
            uniqueKeysWithValues: enabledTokenPreferences(for: .sui).map { entry in
                (
                    entry.contractAddress, SuiBalanceService.KnownTokenMetadata(
                        symbol: entry.symbol, name: entry.name, tokenStandard: entry.tokenStandard, decimals: entry.decimals, marketDataID: entry.marketDataID, coinGeckoID: entry.coinGeckoID
                    )
                )
            }
        )
    ...
    func aptosPackageIdentifier(from value: String?) -> String {
        ...
        guard let package = normalized.split(separator: ":", maxSplits: 1, omittingEmptySubsequences: false).first else { return normalized }
    ...
    var ethereumRPCEndpointValidationError: String? {
        ...
        guard let url = URL(string: trimmedEndpoint), let scheme = url.scheme?.lowercased(), ["http", "https"].contains(scheme), url.host != nil else { return "Enter a valid http or https RPC URL." }
        return nil
    }
    var moneroBackendBaseURLValidationError: String? {
        ...
        guard !trimmedEndpoint.isEmpty else { return nil }
        guard let url = URL(string: trimmedEndpoint), let scheme = url.scheme?.lowercased(), ["http", "https"].contains(scheme), url.host != nil else { return "Enter a valid http or https Monero backend URL." }
        return nil
    }
}
