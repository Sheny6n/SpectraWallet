...
private enum SeedMaterialEnvelope {
    ...
    private static func masterKey() -> SymmetricKey {
        if let storedData = (try? storage.loadData(for: masterKeyAccount)) ?? nil, storedData.count == 32 { return SymmetricKey(data: storedData) }
    ...
    static func encode(_ seedPhrase: String) -> Data {
        let key = masterKey()
        guard let sealedBox = try? AES.GCM.seal(Data(seedPhrase.utf8), using: key) else { return Data(seedPhrase.utf8) }
        let envelope = Envelope(
            version: currentVersion, ciphertext: sealedBox.ciphertext + sealedBox.tag, nonce: Data(sealedBox.nonce)
        )
        return (try? encoder.encode(envelope)) ?? Data(seedPhrase.utf8)
    }
    static func decode(_ data: Data) -> String? {
        if let envelope = try? decoder.decode(Envelope.self, from: data), envelope.version == currentVersion {
            guard envelope.nonce.count == 12, envelope.ciphertext.count >= 16 else { return nil }
            let ciphertext = envelope.ciphertext.dropLast(16)
            let tag = envelope.ciphertext.suffix(16)
            guard let nonce = try? AES.GCM.Nonce(data: envelope.nonce), let sealedBox = try? AES.GCM.SealedBox(nonce: nonce, ciphertext: ciphertext, tag: tag), let plaintext = try? AES.GCM.open(sealedBox, using: masterKey()) else { return nil }
...
enum SecureSeedPasswordStore {
    ...
    private static let encoder = JSONEncoder()
    private static let decoder = JSONDecoder()
    private static func normalizedPassword(_ password: String) -> String { password.trimmingCharacters(in: .whitespacesAndNewlines) }
    private static func randomSalt(length: Int = 16) -> Data { SecureRandom.data(length: length) }
        ...
        let status = salt.withUnsafeBytes { saltBytes in
            passwordData.withUnsafeBytes { passwordBytes in
                CCKeyDerivationPBKDF(
                    CCPBKDFAlgorithm(kCCPBKDF2), passwordBytes.bindMemory(to: Int8.self).baseAddress, passwordData.count, saltBytes.bindMemory(to: UInt8.self).baseAddress, salt.count, CCPseudoRandomAlgorithm(kCCPRFHmacAlgSHA256), UInt32(rounds), &derived, derived.count
                )
            }}
    ...
    static func save(_ password: String, for account: String) throws {
        ...
        let envelope = PasswordVerifierEnvelope(version: currentVersion, salt: salt, rounds: defaultRounds, digest: digest)
    ...
    static func verify(_ password: String, for account: String) -> Bool {
        let normalized = normalizedPassword(password)
        guard !normalized.isEmpty, let data = try? storage.loadData(for: account), let envelope = try? decoder.decode(PasswordVerifierEnvelope.self, from: data), envelope.version == currentVersion else { return false }
