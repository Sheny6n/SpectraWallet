...
enum SeedPhraseSafety {
    ...
    static func validationError(for seedPhrase: String, expectedWordCount: Int? = nil) -> String? {
        ...
        guard !trimmedPhrase.isEmpty else { return "Enter a BIP-39 seed phrase." }
        let words = normalizedWords(from: trimmedPhrase)
        let normalizedPhrase = words.joined(separator: " ")
        if let expectedWordCount, words.count != expectedWordCount { return "This seed phrase has \(words.count) words. Selected length is \(expectedWordCount)." }
        guard validWordCounts.contains(words.count) else { return "Use a valid BIP-39 seed phrase with 12, 15, 18, 21, or 24 words." }
        ...
        guard validateMnemonic(phrase: normalizedPhrase) else { return "This seed phrase is not a valid BIP-39 mnemonic. Check the word spelling and checksum." }
        return nil
    }
    static func hasValidChecksum(_ seedPhrase: String, expectedWordCount: Int? = nil) -> Bool { validationError(for: seedPhrase, expectedWordCount: expectedWordCount) == nil }
    static func generateMnemonic(wordCount: Int) throws -> String { normalizedPhrase(from: _rustGenerateMnemonic(UInt32(wordCount))) }
}
